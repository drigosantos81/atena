...
from sources.selecao_cortes_Copy18 import *
...
class SelecaoLayout():
    ...
    #alterado formato streamlit
    #self.status_consulta_hana
    ...
    opcoes_conf_sm_stc = ['CONVENCIONAL','RECORTE','MISTA']
    ...
    def consulta_selecao(self,b,usuario,senha):
        ...
        self.data_pedido = st.session_state.data_pedido_consultar
        data_pedido = st.session_state.data_pedido_consultar
        #alterado formato streamlit
        #carteira ou sql a ser consultada
        #carteira_sql = self.sql_select.value
        self.carteira_sql = st.session_state.carteira_consultar
        carteira_sql = st.session_state.carteira_consultar
        #alterado formato streamlit
        #Variável para definir se é a tabela de suscetiveis ou sqls 
        consulta = self.consulta_drop.value
        self.consulta = st.session_state.onde_consultar
        
        #alterado formato streamlit
        #self.turma_sm = self.turma_drop.value
        self.turma_sm = st.session_state.turma_consultar
        ...
        st.session_state.quatidade_suscetiveis = len(self.selecao.dfn.index)
    ...
    def importar_clusterconf(self, b):
        ...
        self.carteira_sm = st.session_state.carteira_consultar
        self.turma_sm = st.session_state.turma_consultar
        turma = self.turma_sm
        ...
        sql_conf = sql_conf.format(cart=carteira_import, data = str(st.session_state.data_pedido_consultar),turma = self.turma_sm)
        
        
        #try:
        connection_hana = dbapi.connect(address='BRNEO695',port='30015',user=self.__usuario,password=self.__senha,databasename='BNP',sslValidateCertificate=False)
    ...
    def realizar_selecao_massiva(self, b):
        ...
        st.write("ok 1, nº processos:")
        n_processos = os.cpu_count()
        st.write(n_processos)
        
        #self.nome_selecao = self.nome_arquivo.value
        self.nome_selecao = st.session_state.nome_arquivo_saida
        st.write(self.nome_selecao)
        ...
        try: 
            st.write("ok 3, inicio for zonas_sm...:")
            st.write(zonas_sm.CARTEIRA.unique())
            ...
            if self.erros_sm.shape[0] > 0:
                if 'cluster_id' in self.erros_sm.columns:
                    self.erros_sm = self.erros_sm [['PESO_MTVCOB','PESO_PECLD','PESO_QTDFTVE','LOCALI','MUNICIPIO','BAIRRO','TIPO_LOCAL','SELECIONAR','UTD','ZONA','RAIO_IDEAL','RAIO_MAX','RAIO_STEP','CARTEIRA','TURMA','PREENCHER','QTD_PREENCHER','CLUSTERS','QTD_MAX','QTD_MIN','QTD_POR_CLUSTER','QTD_PREENCHIDA','QTD_TOTAL','PECLD','DIST','cluster_id']]
                else:
