...
class SelecaoCorte():
    ...
    def preencher_cluster(self,UTD: str,r:int,n:int, locali: list = None,zona: list = None, municipio: list = None, bairros: list = None, 
                    ...
                    if resultado.groupby(by="cluster").count()['ZCGACCOUN'].min() >= zona.NUM_CORTES_MIN:
                        ...
                        if raio > zona.RAIO_MAX:
                            ...
                            if zona.PREENCHER != "":
                                ...
                                municipio = municipio, bairros = bairro, min_selecionados=zona.NUM_CORTES_MIN,
                                preencher = zona.PREENCHER, local = zona.TIPO_LOCAL)

                                resultado = resultado.append(preenchidos)
                                resultado['TURMA'] = zona.TURMA
                                resultado[['cluster']] = resultado[['cluster']].apply(lambda x: x + 1)
                                resultado['cluster_id'] = clusterid + resultado['cluster'].astype(str) 
                                n_selecionados_sm = n_selecionados_sm.append(resultado)
                                clusters_erro = resultado.groupby(by="cluster_id").size()
                                erro = zona

                                erro['QTD SELECIONAVEL CLUSTER'] = ",".join(list(map(str,clusters_erro)))  
                                erro['QTD SELECIONAVEL'] = resultado.shape[0]
                                pr = resultado.loc[resultado['SERVICO']==zona.PREENCHER]
                                ...
                                erro['QTD PREENCHER'] = ", ".join([id_preen[i][-1] + ":"+ qtd_preen[i] for i in range(len(id_preen))])
                                ...
                                erro = zona
                                clusters_erro = resultado.groupby(by="cluster_id").size()
                                erro['QTD SELECIONAVEL CLUSTER'] = ",".join(list(map(str,clusters_erro.to_list())))    
                                erro['QTD SELECIONAVEL'] = resultado.shape[0]
                                erro['QTD PREENCHER'] = ""
                                erro['cluster_id'] = (",".join(list(map(str,clusters_erro.index.to_list()))))
                                erros_sm = erros_sm.append(erro)
