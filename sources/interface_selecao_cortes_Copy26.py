from sources.selecao_cortes_Copy17 import *
...
class SelecaoLayout():
    ...
    def set_cluster_id(self):
        ...
        if self.locali !=[""] and self.locali !=" " and self.locali != None:
                clusterid = clusterid + (".".join(self.locali)) 
        return clusterid
    ...
    def importar_clusterconf(self, b):
        ...
        try:
            ...
            conf['TIPO_LOCAL'] = ""
            conf['PREENCHER'] = ""
            conf = conf[['UTD','SELECIONAR','ZONA','LOCALI','MUNICIPIO','BAIRRO','TIPO_LOCAL','CLUSTERS','NUM_CORTES_IDEAL','NUM_CORTES_MIN','RAIO_IDEAL','RAIO_MAX','RAIO_STEP','CARTEIRA','TURMA','PESO_MTVCOB','PESO_PECLD','PESO_QTDFTVE','PREENCHER']]
            #'MIN_QTDFTVE'
    ...
    def realizar_selecao_massiva(self, b):
        ...
        try: 
            ...
            if self.sql_select.value == 'MISTA':
                mista = True
            else:
                mista = False
            ...
            try: 
                ...
                for i in zonas_sm.CARTEIRA.unique():
                    ...
                    if self.multi_process_sm:
                        ...
                        selecionados_sm,n_selecionados_sm,erros_sm, erro = self.selecao.multiprocess_zonas(zonas_sm,self.metodo_sm,self.calcular_irr_sm,self.r_max_preciso_sm,mista)
                ...
                if self.erros_sm.shape[0] > 0:
                    if 'cluster_id' in self.erros_sm.columns:
                        self.erros_sm = self.erros_sm [['PESO_MTVCOB','PESO_PECLD','PESO_QTDFTVE','LOCALI','MUNICIPIO','BAIRRO','TIPO_LOCAL','SELECIONAR','UTD','ZONA','RAIO_IDEAL','RAIO_MAX','RAIO_STEP','CARTEIRA','TURMA','PREENCHER','CLUSTERS','NUM_CORTES_IDEAL','NUM_CORTES_MIN','QTD SELECIONAVEL CLUSTER','QTD SELECIONAVEL','QTD PREENCHER','cluster_id']]
                    else:
                        self.erros_sm = self.erros_sm [['UTD','SELECIONAR','ZONA','LOCALI','MUNICIPIO','BAIRRO','TIPO_LOCAL','RAIO_IDEAL','RAIO_MAX','RAIO_STEP','CLUSTERS','NUM_CORTES_IDEAL','NUM_CORTES_MIN','PESO_MTVCOB','PESO_PECLD','PESO_QTDFTVE','CARTEIRA','TURMA','PREENCHER']]
                if  self.selecionados_sm.shape[0]> 0:
