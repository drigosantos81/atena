...
from sources.selecao_cortes_2_0_teste import *
...
class SelecaoLayout():
    ...
    #Define se quer utilizar o processamento em paralelo ou não, na seleção massiva.
    ...
    #opcoesconf
    opcoes_conf_sm_eps = ['CONVENCIONAL','DISJUNTOR','RECORTE','RECORTE_BAIXA','MISTA']
    opcoes_conf_sm_stc = ['CONVENCIONAL','DISJUNTOR','RECORTE','MISTA']

    opcoes_suscetiveis_stc = ['CONVENCIONAL','DISJUNTOR','RECORTE','MISTA']
    opcoes_suscetiveis_eps = ['CONVENCIONAL','DISJUNTOR','RECORTE','RECORTE_BAIXA','BAIXA MIX','MISTA']
    #Tabela Hana
    ...
    def consulta_selecao(self,b,usuario,senha):
        ...
        try:
            ...
            if consulta == 'Tabela Suscetiveis':
                ...
                self.consulta_nome = self.turma_sm +"_"+carteira_sql
                ...
                self.consulta_nome = self.turma_sm +"_"+ carteira_sql[:-4]
                self.nome_sql.value = self.consulta_nome
                list_confg = ['sources/Sqls/'+carteira_sql]
                carteira_sql = carteira_sql[:-4]
                self.selecao = SelecaoCorte(usuario, senha, consulta= list_confg) 



            if self.turma_drop.value == 'STC':
                if carteira_sql in self.opcoes_conf_sm_stc:
                    self.carteira_sm = carteira_sql
                    self.carteira_conf = carteira_sql
                else:
                    self.carteira_conf = self.opcoes_suscetiveis_stc[0]
            elif self.turma_drop.value == 'EPS':
                if carteira_sql in self.opcoes_conf_sm_eps:
                    self.carteira_sm = carteira_sql
                    self.carteira_conf = carteira_sql
                else:
                    self.carteira_conf = self.opcoes_suscetiveis_eps[0]
            ...
            if 'nome_arquivo' in dir(self): 
                    ...
                    if self.turma_drop.value == 'STC':
                        self.carteira_wgt.options = self.opcoes_conf_sm_stc
                    elif self.turma_drop.value == 'EPS':
                        self.carteira_wgt.options = self.opcoes_conf_sm_eps
                    self.carteira_wgt.value =  self.carteira_conf 
            ...
            if 'SERVICO' in self.selecao.dfn:
                self.mista = True
            else:
                self.mista = False
    ...
    def update_marcacao(self,arquivo):
        try:
            for i in arquivo.MARCACAO.unique():
                if self.marc_insert == 999: self.marc_insert = 0
    ...
    def update_flag_def(self):
        if 'nome_arquivo' not in dir(self):
            ...
            if datetime.now().hour >= 11:
                self.flag_def_wgt.value = 'N'
            else:
                self.flag_def_wgt.value = 'S'
    ...
    def update_marc_insert(self):
        ...
        sql = """SELECT MARCACAO,HORA FROM {tabela} 
                        ...
                        GROUP BY MARCACAO, HORA """.format(tabela=self.tabela_hana)
        ...
        if change['type'] == 'change' and change['name'] == 'value':
            if change['new'] != change['old']:
                if change['new'] == 'Tabela Suscetiveis':
                    self.turma_drop.value = 'STC'
                    self.sql_select.options = self.opcoes_suscetiveis_stc
    ...
    def consulta_turma_change(self,change):
        if change['type'] == 'change' and change['name']=='value':
            if change['new'] != change['old']:
                if change['new'] == 'STC':
                    self.sql_select.options = self.opcoes_suscetiveis_stc
                    #self.carteira_wgt.options = self.opcoes_conf_sm_stc
                    self.sql_select.value = self.opcoes_conf_sm_stc[0]
                elif change['new'] == 'EPS':
                    self.sql_select.options = self.opcoes_suscetiveis_eps
                    #self.carteira_wgt.options = self.opcoes_conf_sm_eps
                    self.sql_select.value = self.opcoes_conf_sm_eps[0]
    ...
    def container_suscetiveis(self):
        ...
        self.container_turma_drop = widgets.HBox(children = [label_turma_drop,self.turma_drop])
        #Observer para mudança da Consulta
        self.consulta_drop.observe(self.consulta_drop_change)
        self.turma_drop.observe(self.consulta_turma_change)
        ...
        sql_select_label = widgets.Label('Selecione Carteira ou SQL:',layout=widgets.Layout(width = '200px', height = '40px', 
                                                                                  margin = conf_pixel))
        self.sql_select = widgets.Dropdown(options = self.opcoes_suscetiveis_stc,layout=widgets.Layout(width = '200px', height = '30px', 
                                                                                  margin = conf_pixel))
    ...
    def set_municipio(self, wdgt):
        ...
        try:
            ...
            if municipio.replace(" ", "") == "" or municipio == None:
                self.municipio = None
    ...
    """"
    def carteira_selecao(self,wdgt):
        try:
            self.carteira_selecao = wdgt.value 
        except:
            self.status_parametros = """ """<label class="status-label error-label">Carteira. Valor inválido.</label>""" """
    """
    ...
    def set_cluster_id(self):
        ...
        if self.utd != " " and self.utd != None:
            clusterid = str(self.utd) + "_" 
        if self.zona != " " and self.zona != None:
            clusterid = clusterid + (".".join(self.zona)) + "_"
        if self.municipio != " " and self.municipio != None:
            clusterid = clusterid + (".".join(self.municipio)) + "_"
        if self.bairros != [""] and self.bairros != " " and self.bairros != None:
                clusterid = clusterid +(".".join(self.bairros)) + "_"
        if self.locali !=[""] and self.locali !=" " and self.locali != None:
                clusterid = clusterid + (".".join(self.locali)) + "_"
        return clusterid
    ...
    def tabela_resultado_sm(self):
        """
        :return: retorna um html com o resultado da seleção massiva
        """
            ...
            if cluster[0] == 'TOTAL':
                ...
                tabela_resultados += """
                                         ...
                                         """.format(cluster[0], cluster[1], cluster[2],
                                                    locale.format("%.2f",cluster[3],grouping = True),
                                                    locale.format("%.2f",mtv_metro_tot / r_tot, grouping = True),
                                                    locale.format("%.2f",cluster[4],grouping = True),
                                                    locale.format("%.2f",pecld_metro_tot / r_tot, grouping = True)
                                                    )
            else:
                if cluster[2] == 0:
                    cluster[2] =1
                ...
                tabela_resultados += """
                                         ...
                                         """.format(cluster[0],
                                                    ...
                                                    cluster[2],
                                                    locale.format("%.2f",cluster[3],grouping = True),
                                                    locale.format("%.2f",mtv_metro,grouping = True),
                                                    locale.format("%.2f",cluster[4],grouping = True),
                                                    locale.format("%.2f",pecld_metro,grouping = True)
                                                    )
    ...
    def importar_clusterconf(self, b):
        ...
        self.importar_conf_sm_text.value = True
        turma = self.turma_sm
        carteira = turma + "_"+self.carteira_sm
        carteira_import = self.carteira_sm
            
        sql_conf = open(self.path_config, encoding="utf-8")  
        sql_conf = sql_conf.read()
        sql_conf = sql_conf.format(cart=carteira_import, data = str(self.date_picker.value),turma = self.turma_sm)
        ...
        #inserido/adicionado no atena novo ---------
        import datetime
        import time
        ...
        query = """
            DELETE FROM CLB344173.MAPA_SELECAO 
            WHERE CARTEIRA = '{var_carteira}'
            AND TURMA = '{var_turma}'
            AND DATA_INSERCAO = '{var_data_insercao}'
            """
        myRow = cursor.execute(query.format(var_carteira=carteira_import,var_turma=turma,var_data_insercao=data_insercao))
        dia_semana = datetime.datetime.today().weekday()
        if turma == "STC":
            if dia_semana == 3:
                data_pedido = date.today() + timedelta(4)
            elif dia_semana == 4:
                data_pedido = date.today() + timedelta(4)
            else:
        ...
        for i in range(len(conf.index)):
            utd = conf["UTD"][i]
            if turma != "STC":
                if utd == "ITABUNA" or utd == "IRECE" or utd == "ALAGOINHAS" or utd == "BARREIRAS" or utd == "FEIRA DE SANTANA NORTE" or utd == "ESPLANADA" or utd == "GRACA" or utd == "ITAPOAN" or utd == "AMARGOSA" or utd == "IPIAU" or utd == "BOM JESUS DA LAPA" or utd == "ILHEUS" or utd == "BRUMADO" or utd == "EUNAPOLIS":    
                    if dia_semana == 3:
            ...
            selecionar = str(conf["SELECIONAR"][i])
            zona = str(conf["ZONA"][i])
            locali = str(conf["LOCALI"][i])
            municipio = str(conf["MUNICIPIO"][i])
            bairro = str(conf["BAIRRO"][i])
            tipo_local = str(conf["TIPO_LOCAL"][i])
            clusters = int(conf["CLUSTERS"][i])
            qtd_max = int(float(conf["QTD_MAX"][i]))
            qtd_min = int(conf["QTD_MIN"][i])
            raio_ideal = int(conf["RAIO_IDEAL"][i])
            raio_max = int(conf["RAIO_MAX"][i])
            raio_step = int(conf["RAIO_STEP"][i])
            carteira = str(conf["CARTEIRA"][i])
            turma = str(conf["TURMA"][i])
            peso_mtvcob = int(conf["PESO_MTVCOB"][i])
            peso_pecld = int(conf["PESO_PECLD"][i])
            peso_qtdftve = int(conf["PESO_QTDFTVE"][i])
            preencher = str(conf["PREENCHER"][i])
            qtd_preencher = 0
            
            insert_list = [(utd,selecionar,zona,locali,municipio,bairro,tipo_local,clusters,qtd_max,qtd_min,raio_ideal,raio_max,raio_step,carteira,turma,peso_mtvcob,peso_pecld,peso_qtdftve,preencher,qtd_preencher,data_insercao,data_pedido)]
            cursor.executemany("""
            INSERT INTO CLB344173.MAPA_SELECAO ("UTD","SELECIONAR","ZONA","LOCALI","MUNICIPIO","BAIRRO","TIPO_LOCAL","CLUSTERS","QTD_MAX","QTD_MIN","RAIO_IDEAL","RAIO_MAX","RAIO_STEP","CARTEIRA","TURMA","PESO_MTVCOB","PESO_PECLD","PESO_QTDFTVE","PREENCHER","QTD_PREENCHER","DATA_INSERCAO","DATA_PEDIDO")
            VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
            """,insert_list)
    ...
    def cluster_hist(self,b):
        ...
        try: 
            flag_defasagem = self.flag_def_wgt.value
            self.set_carteira_sm(self.carteira_wgt)
            data = today
            turma = self.turma_sm
            ...
            if 'cluster' in arquivo_hist.columns:
                ...
                arquivo_hist['cluster'] = arquivo_hist['MARCACAO'].str[-3:].str.lstrip('0')
            ...
            try:
                ...
                #connection_hana.close() 
                #MUDAR DEPOIS PQ N TÁ DANDO CERTO.
                #self.selecao.retirar_contas_selecao(self.selecao.contasselecionadas,arquivo_hist.ZCGACCOUN.tolist())
                #self.selecao_m.retirar_contas_selecao(self.selecao_m.contasselecionadas,arquivo_hist.ZCGACCOUN.tolist()) 
    ...
    def realizar_selecao(self, b):
        ...
        try:
            ...
            for i in self.selecao.selecionados.MARCACAO.unique():
                ...
                self.selecao.selecionados.loc[self.selecao.selecionados['MARCACAO'] ==i,'MARCACAO'] = self.selecao.selecionados.loc[self.selecao.selecionados['MARCACAO']==i]['MARC_ZONA'] + str(self.marc_iter).zfill(3)
        ...
        try:
            if len(self.selecionados_sm.index)> 0:
                if (True in self.selecao.selecionados['ZCGACCOUN'].isin(self.selecionados_sm['ZCGACCOUN']).values):
                    ...
                    selecionados_append['cluster_id'] = selecionados_append['cluster_id'] + selecionados_append['cluster'].astype(str)
                ...
                selecionados_append = self.selecao.selecionados
                selecionados_append[['cluster']] = selecionados_append[['cluster']].apply(lambda x: x+1)
                selecionados_append ['cluster_id'] = clusterid + "_"+selecionados_append['cluster'].astype(str)
                self.selecionados_sm = self.selecionados_sm.append(selecionados_append)
                selecao_nome = self.nome_selecao+".xlsx"
                self.selecionados_sm.to_excel('resultados/'+selecao_nome , index=False)
    ...
    def selecao_gerar_excel(self,b):
        ...
        try:
            ...
            selecao_excel['clusterid'] = selecao_excel['clusterid']  + selecao_excel['cluster'].astype(str)
    ...
    def pesquisar_maiores_devedores(self, b):
        ...
        if self.devedores_locali_text.value:
            locali = self.set_contas_list(self.devedores_locali_text.value.upper())
            df = df.loc[df['ZCGLOCALI'].isin(locali)]
            self.nome_maiores_devedores = self.nome_maiores_devedores + self.devedores_locali_text.value.upper() + "_"    
        ...
        self.df_maiores_devedores = df.copy()
        try:
            df = df[['ZCGACCOUN','ZONA','UTD','ZCGLOCALI','ZCGMUNICI','ZCGBAIRRO','MTV_COB','PECLD','ZCGQTFTVE']].head(self.devedores_qtd_text.value).reset_index()
            ...
            for i in range(df.shape[0]):
                    table += """<tr>
                        ...
                        """.format( df['ZCGACCOUN'].iloc[i],
                                    df['UTD'].iloc[i],
                                    df['ZCGMUNICI'].iloc[i],
                                    df['ZONA'].iloc[i],
                                    df['ZCGLOCALI'].iloc[i],
                                    df['ZCGBAIRRO'].iloc[i],
                                    locale.format("%.2f",round(df['MTV_COB'].iloc[i],2),grouping = True),
                                    locale.format("%.2f",round(df['PECLD'].iloc[i],2),grouping = True),
                                    df['ZCGQTFTVE'].iloc[i])

            self.html_maiores_devedores.value = table
            """    
            ...
            for i in zonas_inserir:
                if self.grupo_sm in self.selecionados_sm:
                    if not(self.selecionados_sm[self.grupo_sm].str.contains(i).any()):
                        ...
                        self.erros_sm.drop(self.erros_sm.loc[self.erros_sm[self.grupo_sm] ==i].index,inplace=True)
            ...
            for cc in contas_inserir:
                if clusterid != 'Nenhum':
                    if not(self.selecionados_sm['ZCGACCOUN'].str.contains(cc).any()):
                        ...
                        if self.mista:
                            conta['CARTEIRA'] = self.selecao.dfn.loc[self.selecao.dfn['ZCGACCOUN']==cc]['SERVICO']
                        try:
            ...
            if (not self.importar_conf_sm):
                if not len(self.arquivo_conf_zonas):
                    ...
                    zonas_sm = pd.read_csv(BytesIO(csv), delimiter=';', 
                                           converters={'ZONA':str,'QTD_MIN':int, 'QTD_MAX':int, 
            ...
            try: 
                ...
                for i in zonas_sm.CARTEIRA.unique():
                    ...
                    if self.multi_process_sm:
                        ...
                        mista = [self.mista]*n_processos
                      
                        res = list(p.imap(self.selecao_m.multiprocess_zonas, divididos,metodo,calcularirr,r_max_preciso,mista))
                        ...
                        selecionados_sm,n_selecionados_sm,erros_sm, erro = self.selecao.multiprocess_zonas(zonas_sm,self.metodo_sm,self.calcular_irr_sm,self.r_max_preciso_sm,self.mista)
                ...
                if self.erros_sm.shape[0] > 0:
                    if 'cluster_id' in self.erros_sm.columns:
                        last_cols = ['CLUSTERS','QTD_MAX','QTD_MIN','QTD_POR_CLUSTER','QTD_PREENCHIDA','QTD_TOTAL','PECLD','DIST','cluster_id']
                        first_cols = [col for col in self.erros_sm.columns if col not in last_cols]

                        self.erros_sm = self.erros_sm[first_cols+last_cols]
                    else:
                        self.erros_sm = self.erros_sm
                        #self.erros_sm = self.erros_sm [['UTD','SELECIONAR','ZONA','LOCALI','MUNICIPIO','BAIRRO','TIPO_LOCAL','RAIO_IDEAL','RAIO_MAX','RAIO_STEP','CLUSTERS','QTD_MAX','QTD_MIN','PESO_MTVCOB','PESO_PECLD','PESO_QTDFTVE','CARTEIRA','TURMA','PREENCHER','QTD_PREENCHER']]
                if  self.selecionados_sm.shape[0]> 0:
    ...
    def maiores_devedores(self):
        ...
        # localidade
        label_devedores_locali = widgets.Label('Localidade:',layout = widgets.Layout(width='15%'))
        self.devedores_locali_text = widgets.Text(layout = widgets.Layout(width = "25%",height='40px'))
        container_devedores_locali = widgets.HBox(children= [label_devedores_locali,
                                                            self.devedores_locali_text])
        ...
        self.html_maiores_devedores=widgets.HTML(value= """""",disabled=True)
        box_maiores = widgets.HBox([self.html_maiores_devedores], layout=widgets.Layout(height='800px', overflow_y='auto'))
        ...
        maiores_devedores_box = widgets.VBox(children=[container_devedores_ordenar,
                                                       ...
                                                       container_devedores_locali,
                                                       container_devedores_bairro,
                                                       container_botoes,
                                                       self.html_link_selecao_maiores_devedores_xlsx,
                                                       box_maiores], layout=widgets.Layout(min_height='500px'))
    ...
    def procurar_clusters(self,b):
        "b: instancia widget"

        clusters = self.get_clusters()
        utd = self.utd_def_text.value
        zona = self.zona_def_text.value
        turma = self.turma_def.value
        carteira = self.carteira_def.value
        marcacao = list(self.marcacao_def_text.value.split(','))

        if utd and utd !="":
            clusters = clusters.loc[clusters['UTD'] ==utd]
        if zona and zona !="":
            clusters = clusters.loc[clusters['ZONA']==zona]
        if turma and turma !="":
            clusters = clusters.loc[clusters['TURMA']==turma]
        if carteira and carteira !="":
            clusters = clusters.loc[clusters['CARTEIRA']==carteira]
        if marcacao and marcacao !=[""]:
            clusters = clusters.loc[clusters['MARCACAO']==marcacao]           
        ...
        self.html_centroides_box.layout.height = '350px'
        self.html_centroides.value = table
    ...
    def defasagem_clusters(self):
        self.html_centroides =widgets.HTML(
            value= """""",
            disabled=True
        )
        self.html_centroides_box = widgets.HBox([self.html_centroides], layout=widgets.Layout(height='0px', overflow_y='auto'))


        self.html_salvar_centroides = widgets.HTML("""""")

        label_realizar_selecao = widgets.HTML(value = '<h1>Realizar Seleção:</h1>',
        layout=widgets.Layout(width = '40%',height = '20px',margin = '0px 40px 40px 4px'))

        label_upload = widgets.Label('Arquivo de Configuração:',layout=widgets.Layout(width ='20%',height = '30px'))
        self.upload_def = widgets.FileUpload(accept='.csv,.xls', multiple=False)

        button_selecionar_def = widgets.Button(description="Realizar seleção",
                                        button_style='success',
                                        layout=widgets.Layout(height='30px', width='30%'))


        container_upload_def = widgets.HBox([label_upload,self.upload_def,button_selecionar_def])
        
        


        label_pesquisar_clusters = widgets.Label('Procurar Clusters:',
                                   layout=widgets.Layout(width= '50%',height = '40px'))

        label_utd = widgets.Label('UTD:',layout=widgets.Layout(width ='20%',height = '30px'))
        self.utd_def_text = widgets.Dropdown(options = self.utd_text.options,layout = widgets.Layout(width ='20%',height = '30px'))
        
        container_utd_def = widgets.HBox([label_utd,self.utd_def_text])

        label_zona = widgets.Label('ZONA:',layout=widgets.Layout(width ='20%',height = '30px'))
        self.zona_def_text = widgets.Dropdown(options = self.zona_text.options, layout = widgets.Layout(width ='20%',height = '30px'))

        container_zona_def = widgets.HBox([label_zona,self.zona_def_text])

        label_marcacao = widgets.Label('Marcação:',layout=widgets.Layout(width ='20%',height = '30px'))
        self.marcacao_def_text = widgets.Text(layout=widgets.Layout(width ='20%',height = '30px'))

        container_marcacao_def = widgets.HBox([label_marcacao,self.marcacao_def_text])

        label_carteira_def = widgets.Label('Carteira:',layout= widgets.Layout(width ='20%',height = '30px'))
        self.carteira_def = widgets.Dropdown(options =[""],layout = widgets.Layout(width ='20%',height = '30px'))
        
        container_carteira_def = widgets.HBox([label_carteira_def,self.carteira_def])

        label_turma_def = widgets.Label('Turma:',layout = widgets.Layout(width ='20%',height = '30px'))
        self.turma_def = widgets.Dropdown(options = self.opcoes_turma_sm + [""],
                                        layout = widgets.Layout(width ='20%',height = '30px'))

        container_turma_def = widgets.HBox([label_turma_def,self.turma_def]) 
        

        self.button_pesquisar_clusters = widgets.Button(layout = widgets.Layout(width = '20%',height = '30px'),
        button_style='info',description = 'Procurar Clusters')
        self.button_salvar_clusters = widgets.Button(layout = widgets.Layout(width = '20%',height = '30px'),
        button_style = 'success',description = 'Salvar Clusters')

        self.button_pesquisar_clusters.on_click(self.procurar_clusters)



        container_buttons = widgets.HBox([self.button_pesquisar_clusters,self.button_salvar_clusters])

        

        defasagem_vbox = widgets.VBox(children = [
                                                #label_realizar_selecao,
                                                container_upload_def,
                                                label_pesquisar_clusters,
                                                container_utd_def,
                                                container_zona_def,
                                                container_marcacao_def,
                                                container_carteira_def,
                                                container_turma_def,
                                                container_buttons,
                                                self.html_salvar_centroides,
                                                self.html_centroides_box
                                                ])
        #defasagem_vbox_hbox = widgets.HBox([defasagem_vbox,html_centroides_box])

        return defasagem_vbox
    ...
    def resultados_hana(self):
        ...
        self.flag_def_wgt = widgets.Dropdown(layout=widgets.Layout(width="116px", height='30px', margin="0px 40px 0px 0px"),
                                                             options=['N','S'],
                                                             value = self.flag_def)
        ...
        #Labels e Widgets
        if self.turma_drop.value == 'STC':
            if self.carteira_sm in self.opcoes_conf_sm_stc:
                valor_carteira = self.carteira_sm
                opcao_carteira = self.opcoes_conf_sm_stc
            else:
                valor_carteira = self.opcoes_conf_sm_stc[0]
        if self.turma_drop.value == 'EPS':
            if self.carteira_sm in self.opcoes_conf_sm_eps:
                valor_carteira = self.carteira_sm
                opcao_carteira = self.opcoes_conf_sm_eps
            else:
                valor_carteira = self.opcoes_conf_sm_eps[0]
        ...
        self.carteira_wgt = widgets.Dropdown(value= self.carteira_conf,layout=widgets.Layout(width="150px", height='30px', margin="0px 20px 0px 0px"),
                                                             options= opcao_carteira )
         #Primeira HBox
        flag_def_box = widgets.HBox(children=[label_carteira_wgt, self.carteira_wgt,
                                             label_flag_def, self.flag_def_wgt])
    ...
    def show(self):
        try:
            ...
            #widgets.HTML(self.style)
            #defasagem_box = widgets.VBox([self.defasagem_clusters()])

            defasagem_box = self.defasagem_clusters()
            ...
            #tab.set_title(index=6,title ="Defasagem")
            tab.set_title(index=6, title="HANA")
  
            return tab
        except Exception as e:
            print("Não foi possível carregar a interface checar quantidade de suscetíveis")
            
            
#Tem q documentar essa brincadeira
