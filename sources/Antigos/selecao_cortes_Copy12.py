...
class SelecaoCorte():
    ...
    def nkcnk(self, UTD: str, locali: list = None,zona: list = None, municipio: list = None, bairros: list = None, n: int = 2,
        ...
        self.validacao = pd.DataFrame()
        self.selecoes = []

        
        #ucs = ucs.loc[~ucs['ZCGACCOUN'].isin(self.contasselecionadas)]
        #ucs = ucs.loc[~ucs['ZCGACCOUN'].isin(self.contasselecionadas_acadaetapa)] 
        if servico and servico !="":
            if servico == 'CORTE':
                ucs = ucs.loc[ucs['ACAO'].isin(['CORTE','DISJ'])]
               
            else:
                ucs = ucs.loc[ucs['ACAO'].isin([servico])]
        ...
        for i in range(n):
            ...
            for index, cc in ccs_lista:
                ...
                self.selecoes += [{
                        'irr_tot': selecionados_aux['irr'].sum(),
                        'pecld_tot': selecionados_aux['PECLD'].sum(),
                        'min_selecionados': min_selecionados if selecionados_aux.shape[0] >= min_selecionados else
                        selecionados_aux.shape[0]
                }]
        ...
        """
        if min_selecionados > 1:
            
            self.selecoes = sorted(selecoes, key=operator.itemgetter('min_selecionados', 'irr_tot'), reverse=True)
            self.selecoes = sorted(selecoes, key=operator.itemgetter('min_selecionados', 'pecld_tot'), reverse=True)
        else:
            self.selecoes_irr = sorted(selecoes, key=lambda i: i['irr_tot'], reverse=True)[0]
            elf.selecoes_pecld =  sorted(selecoes, key=lambda i: i['pecld_tot'], reverse=True)[0]
        """
        
    
        selecionados = self.dfn[self.dfn.ZCGACCOUN.isin(accoun_selecionadas)].copy()
    
        self.teste = selecionados
    ...
    def retirar_contas_selecao(self,var,cc):
        try:
            var.extend(cc)
        except Exception as e:
            print(e)
    ...
    def multiprocess_zonas(self,zonas,metodo,calcularirr,r_max_preciso):
        ...
        erros = []
        self.analise_tot = pd.DataFrame(columns=['UTD','ZONA'])
        for i, zona in zonas.loc[zonas.SELECIONAR=="SIM"].iterrows(): 
            try:
                raio = zona.RAIO_IDEAL 
                zona_concluida = False
                clusterid = ""   
                analise = pd.DataFrame(columns=['UTD','ZONA'])


                if zona.SERVICO and zona.SERVICO !="":
                    clusterid = str(zona.SERVICO[0]) + "_" 
                ...
                if zona.UTD and zona.UTD !="":
                    utd_s = list(zona.UTD.split(','))
                else: 
                    utd_s = None
                ...
                while raio <= zona.RAIO_MAX and zona_concluida == False: 
                    ...
                    if resultado.groupby(by="cluster").count()['ZCGACCOUN'].min() >= zona.NUM_CORTES_MIN:
                        ...
                        analise['ZONA'] = zona_s
                        """
                        sorted_irr= sorted(self.selecoes, key=operator.itemgetter('min_selecionados', 'irr_tot'), reverse=True)[0]
                        sorted_pecld = sorted(self.selecoes, key=operator.itemgetter('min_selecionados', 'pecld_tot'), reverse=True)[0]
                        """
                        analise['IRR'] = resultado['IRR'].sum()
                        analise['PECLD']=resultado['PECLD'].sum()
                        """
                        analise['IRR_irr'] = sorted_irr['irr_tot']
                        analise['IRR_pecld'] = sorted_irr['pecld_tot']
                        analise['PECLD_irr'] = sorted_pecld['irr_tot']
                        analise['PECLD_pecld'] = sorted_pecld['pecld_tot']
                       
                        analise['QTD_SELECIONADOS'] = int(sorted_irr['min_selecionados'])
                        """
                        analise['QTD_SELECIONADOS'] = resultado.shape[0]
                        analise['MIN_OBT'] = 'SIM'
                        self.analise_tot = self.analise_tot.append(analise)
                        ...
                        #print(zona.UTD)
                            ...
                            analise['PECLD_irr'] = sorted_pecld['irr_tot']
                            analise['PECLD_pecld'] = sorted_pecld['pecld_tot']
                            analise['QTD_SELECIONADOS'] = sorted_irr['min_selecionados']
                            """
                            analise['IRR'] = resultado['IRR'].sum()
                            analise['PECLD']=resultado['PECLD'].sum()
                            analise['MIN_OBT'] = 'NAO'
        ...
        return selecionados_sm,n_selecionados_sm,erros_sm,erros,self.analise_tot
