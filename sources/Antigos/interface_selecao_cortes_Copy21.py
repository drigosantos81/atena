from sources.selecao_cortes_Copy13 import *
...
class SelecaoLayout():
    ...
    def importar_clusterconf(self, b):
        ...
        try:
            ...
            conf['TIPO_LOCAL'] = ""
            conf['PREENCHER'] = ""
            conf = conf[['UTD','SELECIONAR','ZONA','LOCALI','MUNICIPIO','BAIRRO','TIPO_LOCAL','CLUSTERS','NUM_CORTES_IDEAL','NUM_CORTES_MIN','RAIO_IDEAL','RAIO_MAX','RAIO_STEP','CARTEIRA','TURMA','SERVICO','PESO_MTVCOB','PESO_PECLD','PESO_QTDFTVE','PREENCHER']]
            #'MIN_QTDFTVE'
    ...
    def realizar_selecao_massiva(self, b):
        ...
        try: 
            ...
            if zonas_sm.CARTEIRA.unique.shape[0]>1:
                mista = True
            else:
                mista = False
            ...
            try: 
                ...
                if self.erros_sm.shape[0] > 0:
                    if 'cluster_id' in self.erros_sm.columns:
                        self.erros_sm = self.erros_sm [['PESO_MTVCOB','PESO_PECLD','PESO_QTDFTVE','LOCALI','MUNICIPIO','BAIRRO','TIPO_LOCAL','SELECIONAR','UTD','ZONA','RAIO_IDEAL','RAIO_MAX','RAIO_STEP','CARTEIRA','TURMA','SERVICO','CLUSTERS','NUM_CORTES_IDEAL','NUM_CORTES_MIN','QTD SELECIONAVEL CLUSTER','QTD SELECIONAVEL','QTD PREENCHER','cluster_id']]
                    else:
                        self.erros_sm = self.erros_sm [['UTD','SELECIONAR','ZONA','LOCALI','MUNICIPIO','BAIRRO','TIPO_LOCAL','RAIO_IDEAL','RAIO_MAX','RAIO_STEP','CLUSTERS','NUM_CORTES_IDEAL','NUM_CORTES_MIN','PESO_MTVCOB','PESO_PECLD','PESO_QTDFTVE','CARTEIRA','TURMA','SERVICO','PREENCHER']]
                if  self.selecionados_sm.shape[0]> 0:
