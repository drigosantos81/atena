from sources.selecao_cortes_Copy12 import *
...
class SelecaoLayout():
    ...
    #Define se quer utilizar o processamento em paralelo na seleção massiva, ou não
    ...
    def importar_clusterconf(self, b):
        ...
        try:
            ...
            conf['BAIRRO'] = ""
            conf['TIPO_LOCAL'] = ""
            conf = conf[['UTD','SELECIONAR','ZONA','LOCALI','MUNICIPIO','BAIRRO','TIPO_LOCAL','CLUSTERS','NUM_CORTES_IDEAL','NUM_CORTES_MIN','RAIO_IDEAL','RAIO_MAX','RAIO_STEP','CARTEIRA','TURMA','PESO_MTVCOB','PESO_PECLD','PESO_QTDFTVE','SERVICO']]
            #'MIN_QTDFTVE'
        ...
        try: 
            ...
            try:
                ...
                if arquivo_hist.shape[0] > 0:
                    print(arquivo_hist.values.tolist())
    ...
    def realizar_selecao_massiva(self, b):
        ...
        try: 
            ...
            self.n_selecionados_sm = pd.DataFrame()
            self.erros_sm = pd.DataFrame()
            analise_tot = pd.DataFrame(columns=['UTD','ZONA'])
            ...
            try: 
                ...
                for i in zonas_sm.CARTEIRA.unique():
                    ...
                    if self.multi_process_sm:
                        ...
                        for i in range(len(res)):
                            ...
                            analise_tot = analise_tot.append(res[i][4])
                        ...
                        selecionados_sm,n_selecionados_sm,erros_sm, erro, analise_tot = self.selecao.multiprocess_zonas(zonas_sm,self.metodo_sm,self.calcular_irr_sm,self.r_max_preciso_sm)
                ...
                if self.erros_sm.shape[0] > 0:
                    if 'cluster_id' in self.erros_sm.columns:
                        self.erros_sm = self.erros_sm [['PESO_MTVCOB','PESO_PECLD','PESO_QTDFTVE','LOCALI','MUNICIPIO','BAIRRO','TIPO_LOCAL','SERVICO','SELECIONAR','UTD','ZONA','RAIO_IDEAL','RAIO_MAX','RAIO_STEP','CARTEIRA','TURMA','CLUSTERS','NUM_CORTES_IDEAL','NUM_CORTES_MIN','QTD SELECIONAVEL CLUSTER','QTD SELECIONAVEL','cluster_id']]
                    else:
                        self.erros_sm = self.erros_sm [['UTD','SELECIONAR','ZONA','LOCALI','MUNICIPIO','BAIRRO','TIPO_LOCAL',    'RAIO_IDEAL','RAIO_MAX','RAIO_STEP','CLUSTERS','NUM_CORTES_IDEAL','NUM_CORTES_MIN','PESO_MTVCOB','PESO_PECLD','PESO_QTDFTVE','CARTEIRA','TURMA','SERVICO']]
                if  self.selecionados_sm.shape[0]> 0:
