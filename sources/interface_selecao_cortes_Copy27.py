from sources.selecao_cortes_Copy16 import *
...
class SelecaoLayout():
    ...
    #carteira seleção Mista
    #implementar depois ->
   
    opcoes_carteira = [""]
    #preencher selecao mistas
    preencher = ""
    #qtd preencher selecao mista
    qtd_preencher = 0
    ...
    #Tabela Suscetiveis
    tabelasuscetiveis = 'SuscetiveisTabela2'
    #opcoes turma
    ...
    opcoes_suscetiveis = ['CONVENCIONAL','DISJUNTOR','RECORTE','BAIXA MIX','RECORTE_BAIXA','MISTA']
    #Tabela Hana
    tabela_hana = 'ETB348056.ATENA_HIST'
    #pathconfig
    path_config = 'sources/Sqls/ImportacaoConfiguracao2.sql'
    #carteira
    ...
    def update_marcacao(self,arquivo):
        try:
            for i in arquivo.MARCACAO.unique():
                ...
                arquivo.loc[arquivo['MARCACAO']==i,'MARCACAO'] = str(today.day).zfill(2) + arquivo.loc[arquivo['MARCACAO']==i]['MARC_ZONA'].str.lstrip("0").str.zfill(3) + str(self.marc_insert)[-3:].zfill(3)
        except Exception as e:
            print(e)
        return arquivo
    ...
    def reset_configs_parametros(self):
        ...
        if self.mista:
            self.carteira = self.selecao.dfn.SERVICO.unique()
        else:
            self.carteira = [""]
            ...
            try: 
                ...
                if self.erros_sm.shape[0] > 0:
                    if 'cluster_id' in self.erros_sm.columns:
                        self.erros_sm = self.erros_sm [['PESO_MTVCOB','PESO_PECLD','PESO_QTDFTVE','LOCALI','MUNICIPIO','BAIRRO','TIPO_LOCAL','SELECIONAR','UTD','ZONA','RAIO_IDEAL','RAIO_MAX','RAIO_STEP','CARTEIRA','TURMA','PREENCHER','QTD_PREENCHER','CLUSTERS','QTD_MAX','QTD_MIN','QTD_POR_CLUSTER','QTD_PREENCHIDA','QTD_TOTAL','cluster_id']]
                    else:
    ...
    def parametros_selecao(self):
        ...
        label_carteira = widgets.Label('Carteira:',layout = widgets.Layout(width='35%'))
        self.carteira_text = widgets.Dropdown(layout = widgets.Layout(width = "20%",height = '30px',value="",
                                                                      options=self.opcoes) 
                                             )
