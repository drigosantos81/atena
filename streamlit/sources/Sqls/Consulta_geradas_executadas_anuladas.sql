...
SELECT TICKET_GERADAS,TICKET_EXECUTADAS,QTD_GERADAS, QTD_EXECUTADAS,QTD_PENDENTES,QTD_ANULADAS,ZCGDTNOTA,DIA_SEMANA,DAYOFMONTH(ZCGDTNOTA) || DIA_SEMANA   as DIA_MES FROM (
...
CASE WHEN DAYOFWEEK(DATA_FORMATADA) = 2 THEN ' (seg)' 
WHEN DAYOFWEEK(DATA_FORMATADA) = 3 THEN ' (ter)' 
WHEN DAYOFWEEK(DATA_FORMATADA) = 4 THEN ' (qua)'
WHEN DAYOFWEEK(DATA_FORMATADA) = 5 THEN ' (qui)'
WHEN DAYOFWEEK(DATA_FORMATADA) = 6 THEN ' (sex)'
WHEN DAYOFWEEK(DATA_FORMATADA) = 7 THEN ' (sab)'
WHEN DAYOFWEEK(DATA_FORMATADA) = 1 THEN ' (dom)'
END AS DIA_SEMANA,
MUNICIPIO,
BAIRRO,
SUM(PECLD) AS PECLD_GERADAS,
CAST(SUM(PECLD)/SUM(QTDE) AS int) AS TICKET_GERADAS,
CAST(SUM(PECLD)/(CASE WHEN SUM(QTDE_EXECUTADAS) = 0 then null else SUM(QTDE_EXECUTADAS) end) AS int) AS TICKET_EXECUTADAS
...
LEFT JOIN (SELECT DISTINCT ZCGACCOUN, ZCGMTVCOB, PECLD_CONS, DATA_CRIACAO 
		 ...
		 WHERE DATA_CRIACAO BETWEEN ADD_DAYS((ADD_MONTHS(NEXT_DAY(LAST_DAY(CURRENT_DATE)),-1)),-30) AND (CURRENT_DATE)
...
WHERE 
ZCGDTNOTA > ADD_DAYS(CURRENT_DATE,-30) 
