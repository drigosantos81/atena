SELECT UTD, TURMA, CARTEIRA, SUM(QTD_EXECUTADAS) AS QTD_EXECUTADAS, SUM(PECLD_EXECUTADAS) AS PECLD_EXECUTADAS, SUM(PECLD_EXECUTADAS)/SUM(QTD_EXECUTADAS) AS TICKET_PECLD_EXECUTADAS FROM (
...
WHERE   (BASE.ZCGTPNOTA IN ('CS','CA') OR (BASE.ZCGTPNOTA = 'CB' AND UPPER(BASE.ZCGTXTCOD) NOT LIKE '%PED%'))
	AND BASE.ZCGDTENCE BETWEEN (ADD_MONTHS(NEXT_DAY(LAST_DAY(CURRENT_DATE)),-1)) AND (CURRENT_DATE)
