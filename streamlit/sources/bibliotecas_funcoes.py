...
over_theme_2 = {'txc_inactive': '#5C881A',

'menu_background':'#FFFFFF', #73A616
'txc_active':'#5C881A', ##5C881A
'option_active':'#FFFFFF', ##FFFFFF
'font-size':'300%',
'font-style':'bold'}

#FFFFFF
over_theme_1 = {'txc_inactive': '#FFFFFF',
'menu_background':'#5C881A',
'txc_active':'#5C881A',
...
def coletar_suscetiveis_total():
  ...
  sql_consulta_suscetiveis = sql_consulta_suscetiveis.format(carteira = "CONVENCIONAL",data_pedido = st.session_state.data_hoje + timedelta(1) , turma = "STC")
...
def get_metas_orcamento_cobranca():
    if 'df_metas' not in st.session_state:
        ...
        mes_hoje_str = str(mes_hoje)
        mes_anterior_str = str(mes_anterior)
        mes_anterior_2_str = str(mes_anterior_2)
        ...
        if mes_hoje == 1:
            ...
            nome_arquivo_anterior_2 = "Dashboard Cobranca Jan"+ano_hoje + "p"
        elif mes_hoje == 3:
            nome_arquivo = "Dashboard Cobranca Mar"+ano_hoje
            nome_arquivo_anterior = "Dashboard Cobranca Fev"+ano_hoje
            nome_arquivo_anterior_2 = "Dashboard Cobranca Fev"+ano_hoje + "p"
        elif mes_hoje == 4:
            ...
            nome_arquivo_anterior_2 = "Dashboard Cobranca Set"+ano_hoje + "p"
            ...
            #nome_arquivo_anterior_2 ="Dashboard Cobranca Out"+ano_hoje 
            ...
            nome_arquivo_anterior_2 = "Dashboard Cobranca Nov"+ano_hoje + "p"
        ...
        try:
            ...
            try:
                ...
                try: 
                    ...
                    print(e)
                    try: 
                        arquivo_dashboard = r'\\BRNEP242\Publico\DDT\PR\SCL\CCO\COGC\Equipe\CRRC\2. Plano de Recuperação de Créditos\\{}.xlsb'.format(nome_arquivo_anterior_2)
                        df_metas = pd.read_excel(arquivo_dashboard,sheet_name = aba_metas)
                        df_orcamento = pd.read_excel(arquivo_dashboard,sheet_name = aba_orcamento)
                        filtro_mes_ref_orcamento = filtro_mes_ref_orcamento_2
                    except Exception as e:
        ...
        df_metas['QTD_META_CORTE'] = df_metas['QTD_META']/dias_uteis_corte
        df_metas['QTD_META_RECORTE'] = df_metas['QTD_META']/dias_uteis_total
...
def get_quadro_geral_utd():
    ...
    #Em feriado alterar st.session_state.data_selecao_hoje para st.session_state.data_selecao_hoje + timedelta(1) 
    ...
    if 'utd_configuracao' not in st.session_state:
        ...
        st.session_state.utd_configuracao = utd_configuracao
    else:
        utd_configuracao = st.session_state.utd_configuracao
    ...
    base_final['QTD_EXECUTADAS'] = base_final['QTD_EXECUTADAS'].fillna(0)
    ...
    st.session_state.horario_quadro_geral_utds = "{}:{}:{}".format(e.hour, e.minute, e.second)
...
def get_quadro_geral_zonas_d_mais_1():
    ...
    # Em feriado alterar  st.session_state.data_selecao_hoje por  st.session_state.data_selecao_hoje+ timedelta(1)
    ...
    if 'nomes_zonas' not in st.session_state:
        ...
        st.session_state.nomes_zonas = nomes_zonas
    else:
        nomes_zonas = st.session_state.nomes_zonas
    ...
    e = datetime.datetime.now()
    st.session_state.horario_quadro_geral_zonas = "{}:{}:{}".format(e.hour, e.minute, e.second)





def formatar_lista_entrada_sql_4():
    ...
    st.session_state.string_lista = ','.join([str(item) for item in lista])
...
def consulta_defasagem_hoje():
    get_metas_orcamento_cobranca()
    if str.lower(st.session_state.pagina_resumo) == "defasagem por zonas" or str.lower(st.session_state.pagina_resumo) == "suscetiveis para mapa":
    ...
    if str.lower(st.session_state.pagina_resumo) == "defasagem por zonas" or str.lower(st.session_state.pagina_resumo) == "suscetiveis para mapa":
        e = datetime.datetime.now()
        st.session_state.horario_defasagem_zonas = "{}:{}:{}".format(e.hour, e.minute, e.second)
        ...
        e = datetime.datetime.now()
        st.session_state.horario_defasagem_utds = "{}:{}:{}".format(e.hour, e.minute, e.second)
...
def coletar_base_reguladas():
    completo_arquivo_gestao_demanda = r'\\clb247340-ba\NPS2\COMERCIAL\RELATÓRIOS\Auxiliar\base_demanda.xlsx'
    data_hoje = date.today()  - timedelta(1) 
    ano_hoje = data_hoje.year 
    mes_carteira = data_hoje.month 
    mes_hoje = data_hoje.month  
    dia_hoje = data_hoje.day 
    dia_hoje = str(dia_hoje) ; mes_hoje = str(mes_hoje) ; ano_hoje = str(ano_hoje) ; mes_carteira = str(mes_carteira) 
    if len(mes_hoje) == 1 : mes_hoje = "0"+ mes_hoje 
    if len(dia_hoje) == 1 : dia_hoje = "0" + dia_hoje
    nome_arquivo_abertas_click = ano_hoje + mes_hoje + dia_hoje +"\CLB_TAREFAS_ABERTAS_AUTORANK_DiaAtu_" + ano_hoje + mes_hoje + dia_hoje    
    completo_arquivo_click_abertas = r'\\BRNEO983\Integracao\Coelba\Engenharia\Click\Saida\Consulta\5Horas\{}.csv.zip'.format(nome_arquivo_abertas_click)
    abertas_click_df = pd.read_csv(completo_arquivo_click_abertas, sep=';',encoding='latin1',compression='zip')
    lista_colunas_click = ['DIA','CALLID','STATUS','TIPODETAREFA','TASKTYPE']
    abertas_click_df = abertas_click_df[lista_colunas_click]
    abertas_click_df['CALLID'] = abertas_click_df['CALLID'].str.slice(2,12) #Deixa as notas sem o tipo da nota no inicio
    abertas_click_df = abertas_click_df.rename(columns={'CALLID':'Nota','DIA':'DIA_ABERTAS','STATUS':'STATUS_ABERTAS'})

    try:
        nome_arquivo_abertas_click_manha = ano_hoje + mes_hoje + dia_hoje +"\CLB_TAREFAS_ABERTAS_AUTORANK_" + ano_hoje + mes_hoje + dia_hoje + "_0559"
        completo_arquivo_click_abertas_manha = r'\\BRNEO983\Integracao\Coelba\Engenharia\Click\Saida\Consulta\Geral\{}.csv.zip'.format(nome_arquivo_abertas_click_manha)
        abertas_manha_click_df = pd.read_csv(completo_arquivo_click_abertas_manha, sep=';',encoding='latin1',compression='zip')
    except Exception as e:
        nome_arquivo_abertas_click_manha = ano_hoje + mes_hoje + dia_hoje +"\CLB_TAREFAS_ABERTAS_AUTORANK_" + ano_hoje + mes_hoje + dia_hoje + "_0600"
        completo_arquivo_click_abertas_manha = r'\\BRNEO983\Integracao\Coelba\Engenharia\Click\Saida\Consulta\Geral\{}.csv.zip'.format(nome_arquivo_abertas_click_manha)
        abertas_manha_click_df = pd.read_csv(completo_arquivo_click_abertas_manha, sep=';',encoding='latin1',compression='zip')

    lista_colunas_click = ['DIA','CALLID','STATUS','TIPODETAREFA','TASKTYPE']
    abertas_manha_click_df = abertas_manha_click_df[lista_colunas_click]
    abertas_manha_click_df['CALLID'] = abertas_manha_click_df['CALLID'].str.slice(2,12) #Deixa as notas sem o tipo da nota no inicio
    abertas_manha_click_df = abertas_manha_click_df.rename(columns={'CALLID':'Nota','DIA':'DIA_ABERTAS_MANHA','STATUS':'STATUS_ABERTAS_MANHA'})

    nome_arquivo_executadas_click = ano_hoje + mes_hoje + dia_hoje +"\CLB_TAREFAS_ENCERRADAS_AUTORANK_DiaAtu_" + ano_hoje + mes_hoje + dia_hoje    
    completo_arquivo_click_executadas = r'\\BRNEO983\Integracao\Coelba\Engenharia\Click\Saida\Consulta\3Horas\{}.csv.zip'.format(nome_arquivo_executadas_click)
    executadas_click_df = pd.read_csv(completo_arquivo_click_executadas, sep=';',encoding='latin1',compression='zip')
    executadas_click_df['CALLID'] = executadas_click_df['CALLID'].str.slice(2,12) #Deixa as notas sem o tipo da nota no inicio
    lista_colunas_click = ['DIA','CALLID','STATUS','STATUSCCS']
    executadas_click_df = executadas_click_df[lista_colunas_click]
    executadas_click_df = executadas_click_df.rename(columns={'CALLID':'Nota','DIA':'DIA_EXEC','STATUS':'STATUS_EXEC'})

    lista_colunas_demanda = ['Nota','Criação','Conclusão desejada','Apontamentos','STATUS PROGRAMAÇÃO (HOJE)','JUSTIFICATIVA','LOG_ERRO']
    demanda_df = pd.read_excel(completo_arquivo_gestao_demanda,sheet_name = 'base_demanda',skiprows = 2)
    demanda_df = demanda_df[lista_colunas_demanda]
    demanda_df['Nota'] = demanda_df['Nota'].astype(str)

    base_reguladas = pd.read_csv('base_reguladas.csv',sep=';',encoding = "ISO-8859-1") 
    base_reguladas.columns = base_reguladas.columns.str.rstrip()
    base_reguladas.columns = base_reguladas.columns.str.lstrip()

    base_reguladas['Nota'] = base_reguladas['Nota'].astype(str)

    base_reguladas = base_reguladas.rename(columns={'ContContr.':'ZCGACCOUN'})
    base_reguladas = pd.merge(base_reguladas, abertas_click_df, how = 'left', on=['Nota'])
    base_reguladas = pd.merge(base_reguladas, executadas_click_df, how = 'left', on=['Nota'])
    base_reguladas = pd.merge(base_reguladas, demanda_df, how = 'left', on=['Nota'])
    base_reguladas = pd.merge(base_reguladas, abertas_manha_click_df, how = 'left', on=['Nota'])
    trim_strings = lambda x: x.strip() if isinstance(x, str) else x
    base_reguladas.applymap(trim_strings)

    base_reguladas['Nota'] = base_reguladas['Nota'].astype(str)
    base_reguladas['Nota'] = base_reguladas['Nota'].astype(str).str.pad(12,'left','0')
    base_reguladas['ZCGACCOUN'] = base_reguladas['ZCGACCOUN'].astype(str)
    base_reguladas['ZCGACCOUN'] = base_reguladas['ZCGACCOUN'].astype(str).str.pad(12,'left','0')

    connection_hana = dbapi.connect(address='BRNEO695',port='30015',user=st.session_state.usuario,password=st.session_state.senha,databasename='BNP',sslValidateCertificate=True)
    sql1 = """
    SELECT TAB004.ZCGQMNUM AS "Nota", PC.ZCGLATITU as LATITUDE,PC.ZCGLONGIT AS LONGITUDE, UTD40 AS UTD, 
    'tipo_1' as TIPO_LAT_LONG
    FROM CLB_CCS_ICC.ZCT_DS_TAB004 TAB004

    LEFT JOIN (
                    SELECT 
                        ZCGACCOUN,ZCGLATITU,ZCGLONGIT, UTD40
                    FROM CLP123432.PLANILHAO_CLIENTE AS PC
                    WHERE 
                        SEQCC = 1
                        AND ZCGLATITU IS NOT NULL
                    ) PC 
                    ON PC.ZCGACCOUN = TAB004.ZCGACCOUN

    WHERE ZCGUSRSTS IN ('CRIA','RLIB','VLIB','REDI','DESP')
    AND ZCGTPNOTA IN ('CL','CC')
    AND ZCGDTCRIA > ADD_DAYS(CURRENT_DATE,-100)
    AND PC.ZCGLATITU IS NOT NULL
    """
    consulta = pd.read_sql(sql1,connection_hana)
    base_reguladas = pd.merge(base_reguladas, consulta, how = 'left', on=['Nota'])
    base_reguladas[['LATITUDE']] = base_reguladas[['LATITUDE']].fillna("0,0")
    for i in range(len(base_reguladas.index)):
        base_reguladas['Rua'].loc[[i]] = str(base_reguladas['Rua'][i]).rstrip()
        base_reguladas['Bairro'].loc[[i]] = str(base_reguladas['Bairro'][i]).rstrip()
        if base_reguladas['LATITUDE'][i] == '0,0':
            var_contrato = base_reguladas['ZCGACCOUN'][i]
            sql2 = """
        SELECT 
                ZCGACCOUN,ZCGLATITU,ZCGLONGIT,UTD40 AS UTD
            FROM CLP123432.PLANILHAO_CLIENTE AS PC
            WHERE 
                ZCGACCOUN  = '{}'
                AND SEQCC = 1 AND ZCGLATITU IS NOT NULL
                            """.format(var_contrato)
            consulta = pd.read_sql(sql2,connection_hana)
            if len(consulta.index) >= 1:
                base_reguladas['LATITUDE'][i] = consulta['ZCGLATITU'][0] 
                base_reguladas['LONGITUDE'][i] = consulta['ZCGLONGIT'][0] 
                base_reguladas['TIPO_LAT_LONG'][i] = "tipo_2_conta_contrato"
                base_reguladas['UTD'][i] = consulta['UTD'][0] 

            else:
                parada = "nao"
                var_bairro = base_reguladas['Bairro'][i]
                for u in range(19):
                    var_rua = base_reguladas['Rua'][i][:-u]

                    
                    if "'" not in var_rua and "'" not in var_bairro:
                        sql3 = """
                        SELECT * FROM (
                    SELECT 
                        ZCGNMLOGR,ZCGBAIRR,CLI001.ZCGACCOUN,ZCGLATITU,ZCGLONGIT,UTD40 AS UTD
                    ...
                    ON PC.ZCGACCOUN = CLI001.ZCGACCOUN
                    WHERE 
                        ZCGNMLOGR IN ('{}')
                        AND ZCGBAIRR IN ('{}')
                    LIMIT 20
                    )
                    WHERE ZCGLATITU IS NOT NULL 
                    LIMIT 1
                                        """.format(var_rua,var_bairro)
                        if parada == "nao":
                            consulta = pd.read_sql(sql3,connection_hana)
                            if len(consulta.index) >= 1:
                                base_reguladas['LATITUDE'][i] = consulta['ZCGLATITU'][0] 
                                base_reguladas['LONGITUDE'][i] = consulta['ZCGLONGIT'][0]
                                base_reguladas['UTD'][i] = consulta['UTD'][0] 
                                parada = "sim"
                                base_reguladas['TIPO_LAT_LONG'][i] = "tipo_3_rua"
    
    base_reguladas = base_reguladas[base_reguladas.LATITUDE != "0,0"]
    base_reguladas['LATITUDE'] = base_reguladas['LATITUDE'].str.replace(',','.')
    base_reguladas['LONGITUDE'] = base_reguladas['LONGITUDE'].str.replace(',','.')
    base_reguladas.to_excel('base_reguladas.xlsx',index=False,encoding='utf-8-sig')
    st.session_state.base_reguladas = base_reguladas
    return st.session_state.base_reguladas


def mapa_reguladas_suscetiveis(base_reguladas):
    lati = -12.9704
    longi = -38.5124
    zoomi = 6

    mapa = folium.Map(width=1400,height=800,location=[lati,longi], tiles="OpenStreetMap", zoom_start=zoomi) 

    fg_reg = folium.FeatureGroup(name = "Reguladas em Aberto") 
    fg_susc = folium.FeatureGroup(name = "Suscetíveis") 

    for k in range (1,5):
        if k == 1: 
            grupo = base_reguladas
            nome = "Reguladas em Aberto"
            cor = 'orange'
        elif k == 2: 
            grupo = base_reguladas
            nome = "Suscetíveis"
        ...
        #elif k == 3: 
            #grupo = regularizacao
            #nome = "Clandestinos Regularizados"
            #cor = 'blue'
        #elif k == 4: 
            #grupo = bairros_clandestinos
            #nome = "Bairros Clandestinos"
            #cor = 'red'
        
        locals()["fg_"+nome] = folium.FeatureGroup(name = nome) 
        
        for i in range(0,len(grupo)):
        
            utd = grupo.iloc[i]['UTD']
            conta_contrato = grupo.iloc[i]['ZCGACCOUN']
            local = grupo.iloc[i]['Local']
            bairro = grupo.iloc[i]['Bairro']
            rua = grupo.iloc[i]['Rua']
            
            
            #medidor = grupo.iloc[i]['MEDIDOR']
            #poste = grupo.iloc[i]['POSTE']
            #trafo = grupo.iloc[i]['TRAFO']
            #sit = grupo.iloc[i]['SITUACAO']
            
            #<b>ID do CID:</b> {idcid} <br>
            #<b>ID do CID:</b> {idcid} <br>
            
            if k==1:
                nota = grupo.iloc[i]['Nota']
                data_cria = grupo.iloc[i]['Dt/criação']
                data_conclusao = grupo.iloc[i]['Concl.desj']
                tipo_nota = grupo.iloc[i]['Texto code para codificação']
        
                legenda = '''
                
                <b>UTD:</b> {utd} <br>
                <b>Conta Contrato:</b> {conta_contrato} <br>
                <b>Nota:</b> {nota} <br>
                <b>Tipo Nota:</b> {tipo_nota} <br>
                <b>Local:</b> {local} <br>
                <b>Bairro:</b> {bairro} <br>
                <b>Rua:</b> {rua} <br>
                <b>Data Criação:</b> {data_cria} <br>
                <b>Data Conclusão:</b> {data_conclusao} <br>
            ...
            iframe = folium.IFrame(legenda,width=210, height=200)
            
            folium.Circle(
                    location=[grupo.iloc[i]['LATITUDE'], grupo.iloc[i]['LONGITUDE']],
                    radius=float(5),
                    popup=folium.Popup(iframe),
                    color=cor,
                    fill=True,
                    fill_color='cor'
                ).add_to(locals()["fg_"+nome])
            
            locals()["fg_"+nome].add_to(mapa)
    ...
    <ul class='legend-labels'>
        <li><span style='background:green;opacity:0.7;'></span>Reguladas</li>
        <li><span style='background:red;opacity:0.7;'></span>Suscetíveis</li>
    ...
    folium_static(mapa,width=1400)
    st.markdown(template_st, unsafe_allow_html=True)
    mapa.save('Mapa Reguladas e Suscetiveis.html')
    #st_data = st_folium(mapa,width=1400)
    #st.markdown(mapa._repr_html_())
...
def quadro_geral_suscetiveis_corte():
    connection_hana = dbapi.connect(address='BRNEO695',port='30015',user=st.session_state.usuario,password=st.session_state.senha,databasename='BNP',sslValidateCertificate=False)
    st.session_state.carteira_suscetiveis = "CORTE"
    st.session_state.visualizacao_suscetiveis = "CONTA CONTRATO"
    st.session_state.turma_suscetiveis = "STC"
    st.session_state.utd_suscetiveis = "TODAS"
    coleta_suscetiveis()
    sql_quadro_geral = """
SELECT SETOR, UTD,SUM(QTD_SUSCETIVEIS) AS "Qtd Suscetíveis",
--MIN(ZCGMTVCOB) AS "Mont Cobrável Minimo",
MIN(PECLD_CONS) AS "PECLD mín" FROM (
SELECT DATAREF, CASE WHEN ACAO = 'DISJ' OR ACAO = 'DISJUNTOR' THEN 'CORTE' ELSE ACAO END AS ACAO,
UTD, COUNT(ZCGACCOUN) AS QTD_SUSCETIVEIS, CAST(MIN(ZCGMTVCOB) AS INT) AS ZCGMTVCOB, CAST(MIN(PECLD_CONS) AS INT) AS PECLD_CONS
FROM CLB961920.TEMP_ATENA
GROUP BY DATAREF,ACAO,UTD)
LEFT JOIN (SELECT DISTINCT utd40,setor40 AS SETOR  FROM CLP123432.PLANILHAO_CLIENTE pc WHERE SEQCC = 1) PC
ON PC.UTD40 = UTD
GROUP BY UTD,SETOR """
    st.session_state.quadro_geral_suscetiveis_stc = pd.read_sql(sql_quadro_geral, connection_hana)  

    st.session_state.turma_suscetiveis = "EPS"
    coleta_suscetiveis()
    sql_quadro_geral = """
SELECT SETOR, UTD,SUM(QTD_SUSCETIVEIS) AS "Qtd Suscetíveis",
--MIN(ZCGMTVCOB) AS "Mont Cobrável Minimo",
...
ON PC.UTD40 = UTD
GROUP BY UTD,SETOR """
    st.session_state.quadro_geral_suscetiveis_eps = pd.read_sql(sql_quadro_geral, connection_hana)  
...
def coleta_suscetiveis():
    ...
    if st.session_state.utd_suscetiveis != "TODAS":
        linha_1 = "WHERE UTD = '{}'".format(st.session_state.utd_suscetiveis)
    else:
        linha_1 = ""
    if st.session_state.visualizacao_suscetiveis == "CONTA CONTRATO" or st.session_state.visualizacao_suscetiveis == "POLIGONAIS" or st.session_state.visualizacao_suscetiveis == "LOCALIDADES":
        cols =['SETOR','ACAO','UTD','ZONA','BASE CLICK','MUNICIPIO','LOCALIDADE','BAIRRO','RUA','CONTA_CONTRATO','NOME_CLIENTE','PARCEIRO','CNAE','MONTANTE','PECLD','SCORE','FAT_VENCIDAS','QTD_CORTE_ANO','QTD_RECORTE_ANO','FLAG_FRAUDE','EVOLUCAO_LEITURA','TEMPO_CORTADO','BX_RENDA','LATITUDE','LONGITUDE']
        linha_0 = "NOME_CLIENTE,PARCEIRO,CNAE,SETOR,ACAO,UTD,ZONA,ZCGMUNICI  AS MUNICIPIO, ZCGLOCALI AS LOCALIDADE, TEMP_ATENA.ZCGBAIRRO AS BAIRRO, RUA, TEMP_ATENA.ZCGACCOUN AS CONTA_CONTRATO, TEMP_ATENA.ZCGMTVCOB AS MONTANTE, PECLD_CONS AS PECLD, SCORE, TEMP_ATENA.ZCGQTFTVE AS FAT_VENCIDAS,   FCC001.ZCGQTCOR AS QTD_CORTE_ANO, QTD_NOTA_CA AS QTD_RECORTE_ANO,FCC001.ZCGFLFRAU AS FLAG_FRAUDE,EVO.EV AS EVOLUCAO_LEITURA,FCC001.ZCGTMPCOR AS TEMPO_CORTADO,CASE WHEN LEFT(pc.ZCGSETIND,2) IN ('BC','BI','BM','BQ','BR') THEN 'S' ELSE 'N' END AS BX_RENDA,LATITUDE, LONGITUDE"
        linha_2 = """
        ORDER BY PECLD_CONS DESC"""
    elif st.session_state.visualizacao_suscetiveis == "RUAS":
        cols = ['SETOR','ACAO','QTD_SUSCETIVEIS','UTD','ZONA','BASE CLICK','MUNICIPIO','LOCALIDADE','BAIRRO','RUA','MONTANTE','PECLD','MEDIA_SCORE','MEDIA_FAT_VENCIDAS']
        linha_0 = "ACAO, COUNT(DISTINCT TEMP_ATENA.ZCGACCOUN) AS QTD_SUSCETIVEIS,UTD,ZONA,ZCGMUNICI  AS MUNICIPIO, ZCGLOCALI AS LOCALIDADE, TEMP_ATENA.ZCGBAIRRO AS BAIRRO, RUA, SUM(TEMP_ATENA.ZCGMTVCOB) AS MONTANTE, SUM(PECLD_CONS) AS PECLD, AVG(SCORE) AS MEDIA_SCORE, AVG(TEMP_ATENA.ZCGQTFTVE) AS MEDIA_FAT_VENCIDAS"
        linha_2 = """GROUP BY ACAO,UTD,ZONA,ZCGMUNICI, ZCGLOCALI , ZCGBAIRRO , RUA
            ORDER BY SUM(PECLD_CONS) DESC, COUNT(DISTINCT TEMP_ATENA.ZCGACCOUN) DESC"""
    elif st.session_state.visualizacao_suscetiveis == "BAIRROS":
        cols = ['SETOR','ACAO','QTD_SUSCETIVEIS','UTD','ZONA','BASE CLICK','MUNICIPIO','LOCALIDADE','BAIRRO','MONTANTE','PECLD','MEDIA_SCORE','MEDIA_FAT_VENCIDAS']
        linha_0 = "SETOR,ACAO, COUNT(DISTINCT TEMP_ATENA.ZCGACCOUN) AS QTD_SUSCETIVEIS,UTD,ZONA,ZCGMUNICI  AS MUNICIPIO, ZCGLOCALI AS LOCALIDADE, TEMP_ATENA.ZCGBAIRRO AS BAIRRO, SUM(TEMP_ATENA.ZCGMTVCOB) AS MONTANTE, SUM(PECLD_CONS) AS PECLD, SUM(PECLD_CONS)/COUNT(DISTINCT TEMP_ATENA.ZCGACCOUN) AS TICKET_PECLD, AVG(SCORE) AS MEDIA_SCORE, AVG(TEMP_ATENA.ZCGQTFTVE) AS MEDIA_FAT_VENCIDAS"
        linha_2 = """GROUP BY ACAO,UTD,ZONA,ZCGMUNICI, ZCGLOCALI , ZCGBAIRRO
            ORDER BY SUM(PECLD_CONS) DESC, COUNT(DISTINCT TEMP_ATENA.ZCGACCOUN) DESC"""
    elif st.session_state.visualizacao_suscetiveis == "MUNICIPIOS":
        cols = ['SETOR','ACAO','QTD_SUSCETIVEIS','UTD','ZONA','BASE CLICK','MUNICIPIO','LOCALIDADE','RUA','MONTANTE','PECLD','MEDIA_SCORE','MEDIA_FAT_VENCIDAS']
        linha_0 = "SETOR,ACAO, COUNT(DISTINCT TEMP_ATENA.ZCGACCOUN) AS QTD_SUSCETIVEIS,UTD,ZONA,ZCGMUNICI  AS MUNICIPIO, ZCGLOCALI AS LOCALIDADE, SUM(TEMP_ATENA.ZCGMTVCOB) AS MONTANTE, SUM(PECLD_CONS) AS PECLD, SUM(PECLD_CONS)/COUNT(DISTINCT TEMP_ATENA.ZCGACCOUN) AS TICKET_PECLD, AVG(SCORE) AS MEDIA_SCORE, AVG(TEMP_ATENA.ZCGQTFTVE) AS MEDIA_FAT_VENCIDAS"
        linha_2 = """GROUP BY ACAO,UTD,ZONA,ZCGMUNICI, ZCGLOCALI 
            ORDER BY SUM(PECLD_CONS) DESC, COUNT(DISTINCT TEMP_ATENA.ZCGACCOUN) DESC"""
    elif st.session_state.visualizacao_suscetiveis == "ZONAS":
        cols = ['SETOR','ACAO','QTD_SUSCETIVEIS','UTD','ZONA','BASE CLICK','MONTANTE','PECLD','MEDIA_SCORE','MEDIA_FAT_VENCIDAS']
        linha_0 = "SETOR,ACAO, COUNT(DISTINCT TEMP_ATENA.ZCGACCOUN) AS QTD_SUSCETIVEIS,UTD,ZONA, SUM(TEMP_ATENA.ZCGMTVCOB) AS MONTANTE, SUM(PECLD_CONS) AS PECLD, SUM(PECLD_CONS)/COUNT(DISTINCT TEMP_ATENA.ZCGACCOUN) AS TICKET_PECLD, AVG(SCORE) AS MEDIA_SCORE, AVG(TEMP_ATENA.ZCGQTFTVE) AS MEDIA_FAT_VENCIDAS"
        linha_2 = """GROUP BY ACAO,UTD,ZONA
            ORDER BY SUM(PECLD_CONS) DESC, COUNT(DISTINCT TEMP_ATENA.ZCGACCOUN) DESC"""
    elif st.session_state.visualizacao_suscetiveis == "UTDS":
        cols = ['SETOR','ACAO','QTD_SUSCETIVEIS','UTD','MONTANTE','PECLD','MEDIA_SCORE','MEDIA_FAT_VENCIDAS']
        linha_0 = "SETOR,ACAO, COUNT(DISTINCT TEMP_ATENA.ZCGACCOUN) AS QTD_SUSCETIVEIS,UTD, SUM(TEMP_ATENA.ZCGMTVCOB) AS MONTANTE, SUM(PECLD_CONS) AS PECLD, SUM(PECLD_CONS)/COUNT(DISTINCT TEMP_ATENA.ZCGACCOUN) AS TICKET_PECLD, AVG(SCORE) AS MEDIA_SCORE, AVG(TEMP_ATENA.ZCGQTFTVE) AS MEDIA_FAT_VENCIDAS"
        linha_2 = """GROUP BY ACAO,UTD
            ORDER BY SUM(PECLD_CONS) DESC, COUNT(DISTINCT TEMP_ATENA.ZCGACCOUN) DESC"""
        linha_1 = ""
    if st.session_state.carteira_suscetiveis == "CONVENCIONAL":
        ...
        from
        (SELECT DATAREF, CASE WHEN ACAO = 'DISJ' OR ACAO = 'DISJUNTOR' THEN 'CORTE' ELSE ACAO END AS ACAO,
    UTD, ZCGACCOUN, ZCGMTVCOB, PECLD_CONS, LATITUDE, LONGITUDE, ZONA, ZCGMUNICI, ZCGLOCALI, TEMP_ATENA.ZCGBAIRRO, SCORE, ZCGQTFTVE, ZCGTIPLOC
    FROM CLB961920.TEMP_ATENA) as TEMP_ATENA
LEFT JOIN (SELECT ZCGNMLOGR AS RUA, ZCGACCOUN FROM CLB_CCS_ICC.ZCT_DS_CLI001 CLI001) AS CLI001
        ON CLI001.ZCGACCOUN =  TEMP_ATENA.ZCGACCOUN
        LEFT JOIN (SELECT ZCGSETIND,ZCGACCOUN,PC.REGIONAL40 AS REGIONAL, PC.SETOR40 AS SETOR, PC.ZCGNOMCLI AS NOME_CLIENTE, pc.ZCGPARTNR AS PARCEIRO,pc.ZCGDSCNAE AS CNAE,pc.ZCGPOSTO AS TRAFO
FROM CLP123432.PLANILHAO_CLIENTE pc WHERE SEQCC = 1) PC 
ON PC.ZCGACCOUN = TEMP_ATENA.ZCGACCOUN
LEFT JOIN CLB_CCS_ICC.ZCT_DS_FCC001 FCC001
ON TEMP_ATENA.ZCGACCOUN = FCC001.ZCGACCOUN 
LEFT JOIN CLB142840.EVO EVO
ON TEMP_ATENA.ZCGACCOUN = EVO.CC 
LEFT JOIN 
(SELECT DISTINCT ZCGACCOUN, SUM(CASE WHEN ZCGTPNOTA = 'CA' THEN 1 ELSE 0 END) AS QTD_NOTA_CA 
FROM CLB_CCS_ICC.ZCT_DS_TAB004 
WHERE (ZCGTPNOTA = 'CA' AND ZCGUSRSTS = 'VREL' AND ZCGDTENCE BETWEEN ADD_DAYS(TO_DATE(CURRENT_DATE),-365) AND CURRENT_DATE)
GROUP BY ZCGACCOUN) AS RECORTE
ON TEMP_ATENA.ZCGACCOUN = RECORTE.ZCGACCOUN	
        {}
        {}
...
ON TEMP_ATENA.ZCGACCOUN = RECORTE.ZCGACCOUN	
    ...
    if st.session_state.visualizacao_suscetiveis != "UTDS":
        if 'nomes_zonas' not in st.session_state:
            nomes_zonas = pd.read_csv('sources/nomes_zonas.csv',sep=';',encoding = "ISO-8859-1") #pegar nomes das zonas...
            nomes_zonas = nomes_zonas[['ZONA','BASE CLICK']]
            nomes_zonas['ZONA'] = nomes_zonas['ZONA'].astype(str).str.pad(4,'left','0')
            ...
            nomes_zonas = st.session_state.nomes_zonas
        st.session_state.base_suscetiveis = pd.merge(st.session_state.base_suscetiveis, nomes_zonas, how = 'left', on=['ZONA'])
    if st.session_state.visualizacao_suscetiveis == "POLIGONAIS":
        sql2 = """
        select ACAO,UTD,ZONA, COUNT(CONTA_CONTRATO) AS QTD_SUSCETIVEIS, SUM(MONTANTE) AS MONTANTE, SUM(PECLD) AS PECLD, AVG(LATITUDE) AS LATITUDE, AVG(LONGITUDE) AS LONGITUDE, NULL AS CONTA_CONTRATO, NULL AS CNAE, NULL AS BX_RENDA, NULL AS RUA, NULL AS BAIRRO,NULL AS MUNICIPIO,
        NULL AS SETOR, NULL AS LOCALIDADE, NULL AS PARCEIRO, NULL AS NOME_CLIENTE, NULL AS SCORE, NULL AS FAT_VENCIDAS, NULL AS QTD_RECORTE_ANO, NULL AS QTD_CORTE_ANO, NULL AS FLAG_FRAUDE, NULL AS EVOLUCAO_LEITURA, NULL AS TEMPO_CORTADO
         FROM (
            {}
            )GROUP BY UTD,ZONA,ACAO
        """.format(sql)
        st.session_state.base_suscetiveis = pd.read_sql(sql2, connection_hana)  
        st.session_state.base_suscetiveis = pd.merge(st.session_state.base_suscetiveis, nomes_zonas, how = 'left', on=['ZONA'])
        cols =['SETOR','ACAO','UTD','ZONA','BASE CLICK','MUNICIPIO','LOCALIDADE','BAIRRO','RUA','CONTA_CONTRATO','NOME_CLIENTE','PARCEIRO','CNAE','MONTANTE','PECLD','SCORE','FAT_VENCIDAS','QTD_CORTE_ANO','QTD_RECORTE_ANO','FLAG_FRAUDE','EVOLUCAO_LEITURA','TEMPO_CORTADO','BX_RENDA','LATITUDE','LONGITUDE','QTD_SUSCETIVEIS']
        st.session_state.base_suscetiveis = st.session_state.base_suscetiveis[cols]
        st.session_state.base_suscetiveis[['ZONA','BASE CLICK']] = st.session_state.base_suscetiveis[['ZONA','BASE CLICK']].fillna("-")
    elif st.session_state.visualizacao_suscetiveis == "LOCALIDADES":
        sql2 = """
        select ACAO,UTD,ZONA, COUNT(CONTA_CONTRATO) AS QTD_SUSCETIVEIS, SUM(MONTANTE) AS MONTANTE, SUM(PECLD) AS PECLD, AVG(LATITUDE) AS LATITUDE, AVG(LONGITUDE) AS LONGITUDE, NULL AS CONTA_CONTRATO, NULL AS CNAE, NULL AS BX_RENDA, NULL AS RUA, NULL AS BAIRRO,NULL AS MUNICIPIO,
        NULL AS SETOR, LOCALIDADE, NULL AS PARCEIRO, NULL AS NOME_CLIENTE, NULL AS SCORE, NULL AS FAT_VENCIDAS, NULL AS QTD_RECORTE_ANO, NULL AS QTD_CORTE_ANO, NULL AS FLAG_FRAUDE, NULL AS EVOLUCAO_LEITURA, NULL AS TEMPO_CORTADO
         FROM (
            {}
            )GROUP BY UTD,ZONA,ACAO,LOCALIDADE
        """.format(sql)
        st.session_state.base_suscetiveis = pd.read_sql(sql2, connection_hana)  
        st.session_state.base_suscetiveis = pd.merge(st.session_state.base_suscetiveis, nomes_zonas, how = 'left', on=['ZONA'])
        cols =['SETOR','ACAO','UTD','ZONA','BASE CLICK','MUNICIPIO','LOCALIDADE','BAIRRO','RUA','CONTA_CONTRATO','NOME_CLIENTE','PARCEIRO','CNAE','MONTANTE','PECLD','SCORE','FAT_VENCIDAS','QTD_CORTE_ANO','QTD_RECORTE_ANO','FLAG_FRAUDE','EVOLUCAO_LEITURA','TEMPO_CORTADO','BX_RENDA','LATITUDE','LONGITUDE','QTD_SUSCETIVEIS']
        ...
        st.session_state.base_suscetiveis = st.session_state.base_suscetiveis[cols]
        st.session_state.base_suscetiveis[['ZONA','BASE CLICK','LOCALIDADE','EVOLUCAO_LEITURA','CNAE']] = st.session_state.base_suscetiveis[['ZONA','BASE CLICK','LOCALIDADE','EVOLUCAO_LEITURA','CNAE']].fillna("-")
        st.session_state.base_suscetiveis[['SCORE','QTD_RECORTE_ANO']] = st.session_state.base_suscetiveis[['SCORE','QTD_RECORTE_ANO']].fillna(0)
        st.session_state.base_suscetiveis[['QTD_RECORTE_ANO']]  = st.session_state.base_suscetiveis[['QTD_RECORTE_ANO']].astype(int)

    st.session_state.base_suscetiveis[['LATITUDE','LONGITUDE']] = st.session_state.base_suscetiveis[['LATITUDE','LONGITUDE']].fillna(0.0)
    return st.session_state.base_suscetiveis
