...
if str(data_hoje.day) == '23' and str(data_hoje.month) == '10' and str(data_hoje.year) =='2022':
    st.session_state.percentual_meta_mes = 0.75 
...
# < MENU ATUAL, ALTERADO EM 24.10.2022 
col_01,col_02, col_03 = st.columns([5,90,5])
col_01.write("")
with col_02:
    st.session_state.pagina = option_menu(None, ["Login", 
"Seleção", 
"Mapas", 
'Suscetíveis',
'Dashboards',
...
'bi bi-info-circle'
    ...
    styles={
     "container": {"margin": "0px", "margin-bottom": "0px"},
        ...
        "nav-link": {"font-color": "#383838","font-size": "15px"},
...
if 1 == 0:
    with st.sidebar: # Menu lateral
        ...
        st.session_state.pagina = option_menu(None, ["Login", "Seleção", "Mapa", "Suscetiveis", "Dashboard","Sobre"],
            ...
            "nav-link": {"font-size": "15px", "text-align": "left", "margin":"0px", "--hover-color": "#93ac46"},
            "nav-link-selected": {"background-color": "#93ac46","color": "#FFFFFF"},
        }
        )   

st.session_state.opcao_setor = ["TODOS","CENTRO",'CENTRO OESTE','CENTRO SUL','METROPOLITANA','NORDESTE','NORTE','OESTE','SALVADOR','SUDOESTE','SUL']
...
if str.lower(st.session_state.pagina) == "seleção":
    with st.sidebar:
        st.session_state.pagina_resumo = option_menu(st.session_state.pagina, ["Defasagem por UTDs", "Defasagem por Zonas", "Seleção D+1 (UTDs)", 'Seleção D+1 (Zonas)'
        ], 
        icons=['calendar-minus', 'calendar-minus', "calendar-plus", 'calendar-plus'
        ], 
            menu_icon="bi bi-table", default_index=0, orientation="vertical",
            styles={
                
                "nav-link-selected": {"background-color": "#5C881A","font-weight": "bold"},
    ...
    if (str.lower(st.session_state.pagina_resumo) == "seleção d+1 (utds)" or str.lower(st.session_state.pagina_resumo) == "seleção d+1 (zonas)") and st.session_state.check_login == 1:
        ...
        with st.sidebar:
            col_def_2,col_def_3 = st.columns([40,60])
            with col_def_2:
            ...
            st.session_state.opcao_filtro_rank = st.selectbox('Filtrar/Rankear por:',  st.session_state.opcao_filtrar_rank)   
        ...
        if str.lower(st.session_state.pagina_resumo) == "seleção d+1 (utds)":
            if st.session_state.turma == 'STC':
                st.session_state.base_final_temp = st.session_state.base_final_temp[['UTD','CARTEIRA','PERCENT_GERA','PERCENT_META','QTD_META','QTD_GERADAS','QTD_EXECUTADAS','QTD_PENDENTES','TOTAL_ESPERADO_MAPA','NOTAS_ESPERADAS_ATENA','QTD_CLUSTER','QTD_SUSCETIVEIS','TICKET_PECLD','DEFASAGEM_MAPA','CONFIG','QTD_ANULADAS','PERCENT_ANULADAS','QTD_META_TOTAL_MENOS_PENDENTE','QTD_META_PARCIAL_MENOS_PENDENTE']]
            else:
        ...
        if str.lower(st.session_state.pagina_resumo) == "seleção d+1 (utds)":
            ...
            workbook.close()
            with st.sidebar:
                col_side_2,col_side_3 = st.columns([50,50])
                with col_side_3:
                ...
                with col_side_2:
                    ...
                    botao_atualizar_d_1_utds = st.button('Atualizar')
                    if botao_atualizar_d_1_utds:
                        get_quadro_geral_utd()
                st.markdown("<h4 style='text-align: right; color: #FF5A00;'> Atualizado em: {}</h4>".format(st.session_state.horario_quadro_geral_utds), unsafe_allow_html=True)
        else:
            worksheet.write_row(start_row, start_col, st.session_state.base_final_temp.columns, header_format)
            for i, column in enumerate(st.session_state.base_final_temp.columns, start=start_col):
                worksheet.write_column(start_row+1, i, st.session_state.base_final_temp[column], text_format)
            ...
            with st.sidebar:
                ...
                st.markdown("<h4 style='text-align: right; color: #FF5A00;'> Atualizado em: {}.</h4>".format(st.session_state.horario_quadro_geral_zonas), unsafe_allow_html=True)
        ...
        if str.lower(st.session_state.pagina_resumo) == "defasagem por zonas":
            ...
            st.session_state.base_final_temp = st.session_state.base_final_temp[['UTD','BASE CLICK','ZONA','CARTEIRA','TOTAL_ESPERADO_MAPA','NOTAS_ESPERADAS_ATENA','NOTAS_GERADAS_TAB4','DEFASAGEM_GERACAO','DEFASAGEM_MAPA','NOTAS_DEFASAGEM_ATENA','DEFASAGEM_PROGRAMACAO','QTD_TURMAS','QTD_NOTAS','JUSTIFICATIVA','QTD_SUSCETIVEIS','TICKET_PECLD','QTD_META','QTD_GERADAS','QTD_EXECUTADAS','QTD_PENDENTES','QTD_ANULADAS','PERCENT_ANULADAS','PERCENT','CONFIG']]
        else:
            ...
            st.session_state.base_final_temp = st.session_state.base_final_temp[['UTD','CARTEIRA','TOTAL_ESPERADO_MAPA','NOTAS_ESPERADAS_ATENA','NOTAS_GERADAS_TAB4','DEFASAGEM_GERACAO','DEFASAGEM_MAPA','NOTAS_DEFASAGEM_ATENA','QTD_SUSCETIVEIS','TICKET_PECLD','QTD_GERADAS','QTD_EXECUTADAS','QTD_PENDENTES','QTD_ANULADAS','PERCENT_ANULADAS','QTD_META','PERCENT_META','PERCENT','CONFIG']]
        hide_table_row_index = """
                <style>
                thead tr th:first-child {display:none}
                tbody th {display:none}
        ...
        with col_def_12:
            ...
            if str.lower(st.session_state.pagina_resumo) == "defasagem por zonas":
                ...
                with st.sidebar:
                    ...
                    with col_side_2:
                        st.write("Atualizar Dados:")
                        botao_atualizar_defasagem = st.button('Atualizar')
                        if botao_atualizar_defasagem:
                            consulta_defasagem_hoje()
                    st.markdown("<h4 style='text-align: right; color: #FF5A00;'> Atualizado em: {}.</h4>".format(st.session_state.horario_defasagem_zonas), unsafe_allow_html=True)
                ...
                with st.sidebar:
                    ...
                    with col_side_2:
                        ...
                        if botao_atualizar_defasagem:
                            consulta_defasagem_hoje()
                    st.markdown("<h4 style='text-align: right; color: #FF5A00;'> Atualizado em: {}.</h4>".format(st.session_state.horario_defasagem_utds), unsafe_allow_html=True)
...
if str.lower(st.session_state.pagina) == "consulta conta contrato" and st.session_state.check_login == 1:
    def consulta_geral_conta_contrato_sql():
        Consulta_lat_long_contas_contratos
...
if str.lower(st.session_state.pagina) == "dashboards":
    
    with st.sidebar:
        st.session_state.pagina_dashboards = option_menu(st.session_state.pagina, ["Notas Geradas"
        ], 
        icons=['bi bi-bar-chart'
        ], 
            menu_icon="bi bi-graph-up", default_index=0, orientation="vertical",
            styles={
    ...
    if str.lower(st.session_state.pagina_dashboards) == "notas geradas":
        st.session_state.opcao_visualizacao = ["TODAS","GERADAS","GERADAS & EXECUTADAS","GERADAS & ANULADAS"]
        st.session_state.opcao_turma_notas_geradas = ["STC","EPS","PRONTIDAO","TODAS"]
        st.session_state.opcao_carteira_notas_geradas = ["CORTE","RECORTE","DISJUNTOR","BAIXA","CORTE & RECORTE"]
        
        with st.sidebar:
            st.markdown("<h4 style='text-align: center; color: #5C881A;'> ANÁLISE GERAÇÃO DE SERVIÇOS DE COBRANÇA</h4>", unsafe_allow_html=True)
            col_side_2,col_side_3 = st.columns([40,60])
           # with col_side_2:
                #st.session_state.data_referencia_geradas_executadas = st.date_input('Data Referencia:',disabled=True) #data hoje
            #with col_side_3:
            st.session_state.visualizacao_notas_geradas = st.selectbox('Opção:', st.session_state.opcao_visualizacao)  
            with col_side_2:
                st.session_state.turma_geradas_executadas = st.selectbox('Turma:', st.session_state.opcao_turma_notas_geradas)  
            with col_side_3:
                st.session_state.carteira_geradas_executadas = st.selectbox('Carteira:', st.session_state.opcao_carteira_notas_geradas)  
            st.session_state.setor_geradas_executadas = st.selectbox('SETOR', st.session_state.opcao_setor)
            st.session_state.utd_geradas_executadas = st.selectbox('UTD:', st.session_state.opcao_utd) 
        if st.session_state.utd_geradas_executadas != "TODAS":   
            connection_hana = dbapi.connect(address='BRNEO695',port='30015',user=st.session_state.usuario,password=st.session_state.senha,databasename='BNP',sslValidateCertificate=False)
            sql_bairros = """SELECT DISTINCT PC.ZCGBAIRR AS BAIRRO FROM CLP123432.PLANILHAO_CLIENTE PC 
    WHERE PC.SEQCC = 1 AND UTD40 = '{}'""".format(st.session_state.utd_geradas_executadas)
            sql_muni = """SELECT DISTINCT PC.MUNICIPIO AS MUNICIPIO FROM CLP123432.PLANILHAO_CLIENTE PC 
WHERE PC.SEQCC = 1 AND UTD40 = '{}'""".format(st.session_state.utd_geradas_executadas)

            st.session_state.base_bairros = pd.read_sql(sql_bairros, connection_hana)
            st.session_state.base_muni = pd.read_sql(sql_muni, connection_hana)
            st.session_state.base_bairros = list(st.session_state.base_bairros['BAIRRO'])
            st.session_state.base_muni = list(st.session_state.base_muni['MUNICIPIO'])
            st.session_state.base_bairros.insert(0, "TODOS")
            st.session_state.base_muni.insert(0, "TODOS")
            with st.sidebar:
                st.session_state.municipio_geradas_executadas = st.selectbox('Municipio:', st.session_state.base_muni) 
                st.session_state.bairro_geradas_executadas = st.selectbox('Bairro:', st.session_state.base_bairros) 
        else:
            st.session_state.municipio_geradas_executadas = "TODOS"
            st.session_state.bairro_geradas_executadas = "TODOS"
        ...
        def coleta_geradas_executadas():
            ...
            consulta = ['sources/Sqls/'+ txt_sql_consulta_geradas_executadas + '.sql',st.session_state.turma_geradas_executadas,st.session_state.carteira_geradas_executadas,st.session_state.utd_geradas_executadas]
            ...
            sql = sql_consulta_geradas_executadas
            connection_hana = dbapi.connect(address='BRNEO695',port='30015',user=st.session_state.usuario,password=st.session_state.senha,databasename='BNP',sslValidateCertificate=False)
            if st.session_state.setor_geradas_executadas != "TODOS" and st.session_state.utd_geradas_executadas == "TODAS":
                sql = """
                {}
                AND SETOR40 = '{}'
                """.format(sql,st.session_state.setor_geradas_executadas)
            if st.session_state.utd_geradas_executadas != "TODAS":   
                sql = """
                {}
                AND UTD40 = '{}'
                """.format(sql,st.session_state.utd_geradas_executadas)
                
            if st.session_state.turma_geradas_executadas != "TODAS":
                sql = """
                {}
                AND TURMA = '{}'
                """.format(sql,st.session_state.turma_geradas_executadas)
            if st.session_state.carteira_geradas_executadas != "TODAS":
                sql = """
                {}
                AND CARTEIRA = '{}'
                """.format(sql,st.session_state.carteira_geradas_executadas)
            if st.session_state.municipio_geradas_executadas != "TODOS":
                sql = """
                {}
                AND MUNICIPIO = '{}'
                """.format(sql,st.session_state.municipio_geradas_executadas)
            if st.session_state.bairro_geradas_executadas != "TODOS":
                sql = """
                {}
                AND BAIRRO = '{}'
                """.format(sql,st.session_state.bairro_geradas_executadas)
            
            st.write(sql)
            st.session_state.base_geradas_executadas = pd.read_sql(sql, connection_hana)
            
            st.session_state.base_geradas_executadas.rename(columns = {
           'QTD_GERADAS':'Qtd. Geradas',
            'QTD_EXECUTADAS':'Qtd. Executadas',
            'QTD_PENDENTES' : 'Qtd. Pendentes',
            'QTD_ANULADAS' : 'Qtd. Anuladas',
            'DIA_MES' : 'Dia',
            'TICKET_GERADAS' : 'Ticket Geradas',
            'TICKET_EXECUTADAS' : 'Ticket Visitadas'
            }, inplace = True)
            st.session_state.base_geradas_executadas_pecld = st.session_state.base_geradas_executadas[['ZCGDTNOTA','Dia','Ticket Geradas','Ticket Visitadas']].copy()
            st.session_state.base_geradas_executadas_pecld = st.session_state.base_geradas_executadas_pecld.groupby(['ZCGDTNOTA','Dia'])[['Ticket Geradas','Ticket Visitadas']].mean().reset_index()
            if st.session_state.visualizacao_notas_geradas == "GERADAS":
                st.session_state.base_geradas_executadas = st.session_state.base_geradas_executadas.groupby(['ZCGDTNOTA','Dia'])[['Qtd. Geradas']].sum().reset_index()
                st.session_state.base_geradas_executadas = st.session_state.base_geradas_executadas[['ZCGDTNOTA','Dia','Qtd. Geradas']]
            elif st.session_state.visualizacao_notas_geradas == "GERADAS & EXECUTADAS":
                st.session_state.base_geradas_executadas = st.session_state.base_geradas_executadas.groupby(['ZCGDTNOTA','Dia'])[['Qtd. Executadas','Qtd. Geradas']].sum().reset_index()
                st.session_state.base_geradas_executadas = st.session_state.base_geradas_executadas[['ZCGDTNOTA','Dia','Qtd. Executadas','Qtd. Geradas']]
            elif st.session_state.visualizacao_notas_geradas == "GERADAS & ANULADAS":
                st.session_state.base_geradas_executadas = st.session_state.base_geradas_executadas.groupby(['ZCGDTNOTA','Dia'])[['Qtd. Anuladas','Qtd. Geradas']].sum().reset_index()
                st.session_state.base_geradas_executadas = st.session_state.base_geradas_executadas[['ZCGDTNOTA','Dia','Qtd. Anuladas','Qtd. Geradas']]
            elif st.session_state.visualizacao_notas_geradas == "PENDENTES":
                st.session_state.base_geradas_executadas = st.session_state.base_geradas_executadas.groupby(['ZCGDTNOTA','Dia'])[['Qtd. Pendentes']].sum().reset_index()
                st.session_state.base_geradas_executadas = st.session_state.base_geradas_executadas[['ZCGDTNOTA','Dia','Qtd. Pendentes']]
            else:
                st.session_state.base_geradas_executadas = st.session_state.base_geradas_executadas.groupby(['ZCGDTNOTA','Dia'])[['Qtd. Executadas','Qtd. Pendentes','Qtd. Anuladas','Qtd. Geradas']].sum().reset_index()
                st.session_state.base_geradas_executadas = st.session_state.base_geradas_executadas[['ZCGDTNOTA','Dia','Qtd. Executadas','Qtd. Pendentes','Qtd. Anuladas','Qtd. Geradas']]
            st.session_state.base_geradas_executadas = st.session_state.base_geradas_executadas.melt(id_vars=['Dia','ZCGDTNOTA'],var_name="Categoria", 
            value_name="Quantidade")
            st.session_state.base_geradas_executadas.sort_values(by=['ZCGDTNOTA'], inplace=True)
            st.session_state.base_geradas_executadas = st.session_state.base_geradas_executadas.reset_index(drop=True)
            
            st.session_state.base_geradas_executadas_pecld = st.session_state.base_geradas_executadas_pecld.melt(id_vars=['Dia','ZCGDTNOTA'],var_name="Categoria", 
            value_name="Valor Selecionado (R$)")
            st.session_state.base_geradas_executadas_pecld.sort_values(by=['ZCGDTNOTA'], inplace=True)
            st.session_state.base_geradas_executadas_pecld = st.session_state.base_geradas_executadas_pecld.reset_index(drop=True)
            
            e = datetime.datetime.now()
            st.session_state.horario_grafico_notas_geradas = "{}:{}:{}".format(e.hour, e.minute, e.second)
        def coleta_geradas_executadas_pendentes():
            txt_sql_consulta_geradas_executadas = 'Consulta_geradas_executadas_anuladas_total_pendentes'
            consulta = ['sources/Sqls/'+ txt_sql_consulta_geradas_executadas + '.sql',st.session_state.turma_geradas_executadas,st.session_state.carteira_geradas_executadas,st.session_state.utd_geradas_executadas]
            arquivo_sql_consulta_geradas_executadas = open(consulta[0], encoding="utf-8")
            sql_consulta_geradas_executadas = arquivo_sql_consulta_geradas_executadas.read()
            arquivo_sql_consulta_geradas_executadas.close()
            ...
            sql = """
                {}
                GROUP BY MONTH(ZCGDTNOTA) || YEAR(ZCGDTNOTA)
                """.format(sql)
                
            st.session_state.base_geradas_pendentes = pd.read_sql(sql, connection_hana)
            
            st.session_state.base_geradas_pendentes.rename(columns = {
            'QTD_PENDENTES' : 'Qtd. Pendentes',
            'MES_ANO' : 'MES_ANO'
            }, inplace = True)
            st.session_state.base_geradas_pendentes = st.session_state.base_geradas_pendentes.groupby(['MES_ANO'])[['Qtd. Pendentes']].sum().reset_index()
            st.session_state.base_geradas_pendentes = st.session_state.base_geradas_pendentes[['MES_ANO','Qtd. Pendentes']]
            st.session_state.base_geradas_pendentes = st.session_state.base_geradas_pendentes.melt(id_vars=['MES_ANO'],var_name="Categoria", 
            value_name="Quantidade")
            st.session_state.base_geradas_pendentes = st.session_state.base_geradas_pendentes.reset_index(drop=True)
            

        
        with st.sidebar:
            st.write("Atualizar Gráfico:")
            botao_atualizar_grafico_notas_geradas = st.button('Atualizar')
            if botao_atualizar_grafico_notas_geradas:
                coleta_geradas_executadas()
                #coleta_geradas_executadas_pendentes()
            if 'horario_grafico_notas_geradas' in st.session_state:
                st.markdown("<h4 style='text-align: right; color: #FF5A00;'> Atualizado em: {}</h4>".format(st.session_state.horario_grafico_notas_geradas), unsafe_allow_html=True)
        



        # Primeiro gráfico, formato tradicional, Geradas X Visitadas
        ...
        #rects2 = ax.bar(x + width/2, qtd_executadas, width, label='Visitadas')
        ...
        #with col_graf3:
        #st.pyplot(fig)
        #Final Primeiro gráfico, formato tradicional, Geradas X Visitadas

        if 1 == 1:
            lista_sort = list(st.session_state.base_geradas_executadas['Dia'])
            nearest = alt.selection(type='single', nearest=True, on='mouseover',
                            fields=['ZCGDTNOTA','DIA_MES'], empty='none')
            line = alt.Chart(st.session_state.base_geradas_executadas,title="Status Notas Geradas").mark_line().encode(
                alt.X('Dia:O',sort=lista_sort),
                y='Quantidade:Q',
                color='Categoria:N'
            )
            selectors = alt.Chart(st.session_state.base_geradas_executadas).mark_point().encode(
                alt.X('Dia:O',sort=lista_sort),
                opacity=alt.value(0),
            ).add_selection(
                nearest
            ...
            text = line.mark_text(align='center', dx=5, dy=-10).encode(
                text='Quantidade'
            )
            alt.layer(
                line, selectors, points, text
            )
            st.altair_chart(line + points + selectors + text , 
        use_container_width=True)

            lista_sort = list(st.session_state.base_geradas_executadas_pecld['Dia'])
            nearest = alt.selection(type='single', nearest=True, on='mouseover',
                            fields=['ZCGDTNOTA','DIA_MES'], empty='none')
            line = alt.Chart(st.session_state.base_geradas_executadas_pecld,title="Ticket (R$) Notas Geradas").mark_line().encode(
                alt.X('Dia:O',sort=lista_sort),
                y='Valor Selecionado (R$):Q',
                color='Categoria:N'
            )
            selectors = alt.Chart(st.session_state.base_geradas_executadas_pecld).mark_point().encode(
                alt.X('Dia:O',sort=lista_sort),
                opacity=alt.value(0),
            ).add_selection(
                nearest
        ...
        use_container_width=True)
            if 1 == 0:
                lista_sort = list(st.session_state.base_geradas_pendentes['MES_ANO'])
                nearest = alt.selection(type='single', nearest=True, on='mouseover',
                                fields=['MES_ANO'], empty='none')
                line = alt.Chart(st.session_state.base_geradas_pendentes,title="Status Notas Geradas").mark_line().encode(
                    alt.X('MES_ANO:O',sort=lista_sort),
                    y='Quantidade:Q',
                    color='Categoria:N'
                )
                selectors = alt.Chart(st.session_state.base_geradas_pendentes).mark_point().encode(
                    alt.X('MES_ANO:O',sort=lista_sort),
                    opacity=alt.value(0),
                ).add_selection(
                    nearest
        ...
        if 1 == 0:
            total_teste = st.session_state.base_geradas_executadas.groupby(['ZCGDTNOTA','DIA_MES'])[['QTD_EXECUTADAS','QTD_PENDENTES','QTD_ANULADAS']].sum().reset_index()

            total_teste = total_teste[['ZCGDTNOTA','DIA_MES','QTD_EXECUTADAS','QTD_PENDENTES','QTD_ANULADAS']]
            total_teste = total_teste.melt(id_vars=['DIA_MES','ZCGDTNOTA'],var_name="Categoria", 
            value_name="y")
            total_teste.sort_values(by=['ZCGDTNOTA'], inplace=True)
            total_teste = total_teste.reset_index(drop=True)
            lista_sort = list(total_teste['DIA_MES'])
            base = alt.Chart(total_teste).encode(
            alt.X('DIA_MES:O',sort=lista_sort),
            alt.Y('y:Q', stack=True),)
            chart = base.mark_bar().encode(
            color='Categoria:N'
        ).interactive()
            text = base.mark_text(
                align='center',baseline='middle',color='black', dx=0, dy=0).encode(
                alt.X('DIA_MES:O',sort=lista_sort),
                alt.Y('y:Q'),
                text=alt.Text('y:Q'))
            alt.layer(
...
if str.lower(st.session_state.pagina) == "suscetíveis":
    ...
    with st.sidebar:
        st.session_state.pagina_suscetiveis = option_menu(st.session_state.pagina, ["Resumo","Suscetiveis", "Suscetiveis para Mapa"
        ], 
        icons=['gear-fill','bi bi-cash-coin', 'table'
        ], 
            menu_icon="bi bi-scissors", default_index=0, orientation="vertical",
            styles={
    ...
    if str.lower(st.session_state.pagina_suscetiveis) == "resumo":
        st.session_state.opcao_turma_resumo_suscetiveis = ["STC","EPS"]
        st.session_state.opcao_carteira_resumo_suscetiveis = ["CORTE"]
        
        with st.sidebar:
            col_def_2,col_def_3 = st.columns([40,60])
            with col_def_2:
                st.session_state.turma = st.selectbox('Turma:', st.session_state.opcao_turma_resumo_suscetiveis)  
            with col_def_3:
                st.session_state.carteira = st.selectbox('Carteira:', st.session_state.opcao_carteira_resumo_suscetiveis)  
            st.session_state.utd_resumo_suscetiveis = st.selectbox('UTD:', st.session_state.opcao_utd) 
            st.session_state.setor_resumo_suscetiveis = st.selectbox('SETOR:', st.session_state.opcao_setor)
            st.session_state.data_referencia_suscetiveis = st.date_input('Data Referencia:',disabled=True) #data hoje
            st.write("Atualizar Dados:")
            botao_atualizar_resumo = st.button('Atualizar')
            if botao_atualizar_resumo:
                with st.spinner('Carregando...'):
                    quadro_geral_suscetiveis_corte()
        if 'quadro_geral_suscetiveis_stc' not in st.session_state:
            with st.spinner('Carregando...'):
                quadro_geral_suscetiveis_corte()

        if st.session_state.turma == 'STC':
            st.session_state.base_suscetiveis_temp = st.session_state.quadro_geral_suscetiveis_stc
        else:
            st.session_state.base_suscetiveis_temp = st.session_state.quadro_geral_suscetiveis_eps
        if st.session_state.utd_resumo_suscetiveis != 'TODAS':
            st.session_state.base_suscetiveis_temp = st.session_state.base_suscetiveis_temp[(st.session_state.base_suscetiveis_temp['UTD'] == st.session_state.utd_resumo_suscetiveis)]
        if st.session_state.setor_resumo_suscetiveis != 'TODOS':
            st.session_state.base_suscetiveis_temp = st.session_state.base_suscetiveis_temp[(st.session_state.base_suscetiveis_temp['SETOR'] == st.session_state.setor_resumo_suscetiveis)]
        st.table(st.session_state.base_suscetiveis_temp)
    
    elif str.lower(st.session_state.pagina_suscetiveis) == "suscetiveis":
        ...
        st.session_state.opcao_carteira_suscetiveis = ["CORTE","RECORTE","DISJUNTOR","BAIXA","CORTE & RECORTE"]
        ...
        with st.spinner('Carregando...'):
            if st.session_state.carteira_suscetiveis != "CORTE & RECORTE":
                coleta_suscetiveis()
            else:
                st.session_state.carteira_suscetiveis = "CORTE"
                coleta_suscetiveis()
                st.session_state.base_suscetiveis_corte = st.session_state.base_suscetiveis
                st.session_state.carteira_suscetiveis = "RECORTE"
                coleta_suscetiveis()
                st.session_state.base_suscetiveis = pd.concat([st.session_state.base_suscetiveis_corte, st.session_state.base_suscetiveis])
                st.session_state.carteira_suscetiveis = "CORTE & RECORTE"
...
if str.lower(st.session_state.pagina) == "mapas":

    with st.sidebar:
        st.session_state.pagina_mapa = option_menu(st.session_state.pagina, ["Mapa Suscetíveis", "Mapa Seleção"
        ], 
        icons=['bi bi-cash-coin', 'table'
        ], 
            menu_icon="geo-alt", default_index=0, orientation="vertical",
            styles={
    ...
    if st.session_state.pagina_mapa == "Mapa Seleção":
        ...
        st.session_state.opcao_turma = ["STC","EPS"]
        st.session_state.opcao_carteira = ["TODAS","CORTE","RECORTE","DISJUNTOR","BAIXA"]
        st.session_state.opcao_defasagem = ["N","S"]
        ...
        with st.sidebar:
            ...
            col_side_1,col_side_2,col_side_3 = st.columns([58,70,50])
            with col_side_1:
                st.session_state.turma = st.selectbox('Turma:', st.session_state.opcao_turma)  
            with col_side_2:
                st.session_state.carteira = st.selectbox('Carteira:', st.session_state.opcao_carteira) 
            with col_side_3:
                 st.session_state.plotar_defasagem = st.selectbox('Defasagem:', st.session_state.opcao_defasagem) 
            

            st.session_state.opcao_nao_sim = ["NÃO","SIM"]
            col_side_11,col_side_22,col_side_33 = st.columns([65,50,50])
            with col_side_11:
                st.session_state.data_pedido_folium = st.date_input('Data Pedido:',disabled=False) #data hoje
            with col_side_22:
                st.session_state.exibir_legenda = st.selectbox('Legenda:', st.session_state.opcao_nao_sim)  
            with col_side_33:
                st.session_state.exibir_mini_mapa = st.selectbox('Mini Mapa:', st.session_state.opcao_nao_sim)

            st.session_state.utd_folium = st.selectbox('UTD:', st.session_state.opcao_utds) 
            st.session_state.lista_entrada = st.text_input("LISTA DE CONTRATOS PARA PLOTAR:", help='Cole a lista das contas contratos:')
        ...
        #with st.sidebar:
            ...
            if len(base_df) > 0:
                cl0,clll1,clll2,clll3,clll4,clll1_1  = st.columns([5,19,19,19,19,19])
                cl0.write("")
        ...
        with st.sidebar:
            ...
            with clll1_11:
                st.session_state.larg_mapa = st.slider('Largura do Mapa', min_value=1000, max_value=1600, step=10,value=1400)
        ...
        #base_df
            ...
            col_def_01,col_def_1  = st.columns([5,95])
            with col_def_1:
                if len(base_df) > 0:
        ...
        """
                    ...
                    tit_0 = "<li><span style='background:#5C881A;'></span> Pacote 1</li>"
                    tit_1 = "<li><span style='background:#0070C0;'></span> Pacote 2</li>"
                    tit_2 = "<li><span style='background:#FFC000;'></span> Pacote 3</li>"
                    tit_3 = "<li><span style='background:#FF5A00;'></span> Pacote 4</li>"

                    legend_mapa_0 = """

                    <div class='my-legend'>"""
                    legend_mapa_1 = "<div class='legend-title'> Legenda Mapa UTD {} ({}) ({}) </div>".format(str.lower(st.session_state.utd_folium),st.session_state.turma,st.session_state.data_pedido_folium)
            ...
            <style type='text/css'>
            .my-legend .legend-title {text-align: left;margin-bottom: 8px;margin-left: 12px;font-size: 90%;}
            .my-legend .legend-scale ul {margin: 0;padding: 0;float: left;list-style: none;}
            .my-legend .legend-scale ul li {display: block;float: left;width: 90px;margin-bottom: 3px;text-align: center;font-size: 80%;list-style: none;}
            .my-legend ul.legend-labels li span {display: block;float: left;height: 15px;width: 95px;}
            .my-legend .legend-source {font-size: 80%;color: #999;clear: both;}
            ...
            """
                    ...
                    .maplegend ul.legend-labels li span {
                        display: block;
                        float: left;height: 16px;width: 30px;margin-right: 5px;margin-left: 0;border: 1px solid #999;}
    ...
    if st.session_state.pagina_mapa == "Mapa Seleção":
        formatar_lista_entrada_sql_4()
        #st.write(st.session_state.string_lista)
        def consulta_lat_long_contas_contratos():
            txt_sql_consulta_geral = 'Consulta_lat_long_contas_contratos'
            consulta = ['sources/Sqls/'+ txt_sql_consulta_geral + '.sql', st.session_state.string_lista]
            arquivo_sql_consulta_geral = open(consulta[0], encoding="utf-8")
            sql_consulta_geral = arquivo_sql_consulta_geral.read()
            arquivo_sql_consulta_geral.close()
            sql_consulta_geral = sql_consulta_geral.format(lista_consultar = consulta[1])
            connection_hana = dbapi.connect(address='BRNEO695',port='30015',user=st.session_state.usuario,password=st.session_state.senha,databasename='BNP',sslValidateCertificate=False)
            st.session_state.base_plotar = pd.read_sql(sql_consulta_geral, connection_hana)
            st.session_state.base_plotar['LATITUDE'] = st.session_state.base_plotar['LATITUDE'].str.replace(',','.')
            st.session_state.base_plotar['LONGITUDE'] = st.session_state.base_plotar['LONGITUDE'].str.replace(',','.')
        consulta_lat_long_contas_contratos()    
        if len(st.session_state.base_plotar) > 0: 
            #st.write(st.session_state.base_plotar)
            st.session_state.base_plotar['PECLD'] = st.session_state.base_plotar['PECLD'].fillna(0.0)
            for i, row in st.session_state.base_plotar.iterrows():            
                ...
                nivel_advertencia = row['ZCGNNVLAD']
                cnae = row['CNAE']
                ...
                legenda = '''
                    ...
                    '''.format(utd,nivel_advertencia,conta_contrato,municipio,bairro,poligonal,pecld,latitude,longitude)
                iframe = folium.IFrame(legenda,width=380, height=230)
                
                folium.Circle(
                    
                location=[float(row['LATITUDE']),float(row['LONGITUDE'])],
                radius=float(raio_circulo),
                popup=folium.Popup(iframe),
                color=cor,
                fill_opacity=1,
                fill=True,
        ...
        #st.session_state.opcao_visualizacao = ["CONTA CONTRATO","RUAS","BAIRROS","MUNICIPIOS","ZONAS","UTDS"]
        st.session_state.opcao_visualizacao = ["CONTA CONTRATO","POLIGONAIS","LOCALIDADES"]
        st.session_state.opcao_turma_suscetiveis = ["STC","EPS"]
        st.session_state.opcao_carteira_suscetiveis = ["CORTE","RECORTE","DISJUNTOR","BAIXA","CORTE & RECORTE"]
        ...
        with st.sidebar:
            ...
            col_side_2,col_side_3 = st.columns([40,60])
            with col_side_2:
                st.session_state.data_referencia_suscetiveis = st.date_input('Data Referencia:',disabled=False) #data hoje
            with col_side_3:
                st.session_state.visualizacao_suscetiveis = st.selectbox('Opção:', st.session_state.opcao_visualizacao)  
        ...
        if botao_consulta_suscetiveis:
            st.session_state.turma_suscetiveis_consulta = st.session_state.turma_suscetiveis
            st.session_state.utd_suscetiveis_consulta = st.session_state.utd_suscetiveis
            st.session_state.carteira_suscetiveis_consulta = st.session_state.carteira_suscetiveis
            if st.session_state.carteira_suscetiveis != "CORTE & RECORTE":
                with st.spinner('Carregando...'):
                    coleta_suscetiveis()
            else:
        ...
        if 'base_suscetiveis'  in st.session_state :
            if not len(st.session_state.base_suscetiveis) > 0:
                st.error("Sem base de suscetíveis para o dia selecionado!")
            #st.write(st.session_state.base_suscetiveis)
            ...
            if int(total_total) > 1000:
                raio_circulo = 20
                tam_quadrado = 0.0002
            elif int(total_total) > 800:
                raio_circulo = 25
                tam_quadrado = 0.0003
            elif int(total_total) > 600:
                raio_circulo = 30
                tam_quadrado = 0.0004
            elif int(total_total) > 300:
                raio_circulo = 30
                tam_quadrado = 0.0005
            else:  
                raio_circulo = 35
            ...
            valor_maximo_pecld = max(base_df['PECLD'])
            ...
            for i, row in base_df.iterrows():            
                ...
                if st.session_state.visualizacao_suscetiveis == "POLIGONAIS" or st.session_state.visualizacao_suscetiveis == "LOCALIDADES":
                     
                    qtd_suscetiveis = row['QTD_SUSCETIVEIS']
                    ticket_medio = int(pecld/qtd_suscetiveis)
                    
                    if st.session_state.visualizacao_suscetiveis == "POLIGONAIS":
                        legenda = '''
                        <b>UTD:</b> {} <br>
                        <b>Zona:</b> {} <br>
                        <b>Acao:</b> {} <br>
                        <b>Qtd. Suscetíveis:</b> {} <br>
                        
                        <b>PECLD:</b> R${} <br>
                        <b>Ticket médio:</b> R${} <br>
                        '''.format(utd,poligonal,acao,qtd_suscetiveis,pecld,ticket_medio)
                        html_mapa = '<div style="font-size: 8pt;color:#383838">{} <br> ticket: R${} <br> suscetíveis: {}</div>'.format(str.lower(poligonal),ticket_medio,qtd_suscetiveis)
                    else:
                        legenda = '''
                        <b>UTD:</b> {} <br>
                        <b>Zona:</b> {} <br>
                        ...
                        <b>PECLD:</b> R${} <br>
                        <b>Ticket médio:</b> R${} <br>
                        '''.format(utd,poligonal,local,acao,qtd_suscetiveis,pecld,ticket_medio)
                        html_mapa = '<div style="font-size: 8pt;color:#383838">{} - {} <br> ticket: R${} <br> suscetíveis: {}</div>'.format(str.lower(local),str.lower(poligonal),ticket_medio,qtd_suscetiveis)


                    iframe = folium.IFrame(legenda,width=380, height=230)

                    if row['PECLD']/valor_maximo_pecld < 0.11:
                        raio_circulo = 3000*0.1
                    else:
                        raio_circulo = 3000*(row['PECLD']/valor_maximo_pecld)
                    folium.Circle(
    ...
    icon=DivIcon(
        ...
        html=html_mapa,
                    ...
                    if acao == "RECORTE":
                        folium.Rectangle(
                            bounds=[[row['LATITUDE'],row['LONGITUDE']],[row['LATITUDE']-tam_quadrado,row['LONGITUDE']],[row['LATITUDE']-tam_quadrado,row['LONGITUDE']-tam_quadrado],[row['LATITUDE'],row['LONGITUDE']-tam_quadrado]],
                        location=[row['LATITUDE'],row['LONGITUDE']],
                        radius=float(raio_circulo),
                        popup=folium.Popup(iframe),
                        color=cor,
            ...
            if 'utd_suscetiveis_consulta'  in st.session_state:
                if st.session_state.visualizacao_suscetiveis == "POLIGONAIS":
                    nome_legenda = "Poligonais"
                    legend_mapa_1 = "<div class='legend-title'> Legenda Mapa UTD {} ({}) - {} poligonais (Carteira: {}) </div>".format(str.lower(st.session_state.utd_suscetiveis_consulta),st.session_state.turma_suscetiveis_consulta,total_total,st.session_state.carteira_suscetiveis_consulta)
                elif st.session_state.visualizacao_suscetiveis == "LOCALIDADES":
                    nome_legenda = "Localidades"
                    legend_mapa_1 = "<div class='legend-title'> Legenda Mapa UTD {} ({}) - {} poligonais (Carteira: {}) </div>".format(str.lower(st.session_state.utd_suscetiveis_consulta),st.session_state.turma_suscetiveis_consulta,total_total,st.session_state.carteira_suscetiveis_consulta)
                else:
                    nome_legenda = "Contratos"
                    legend_mapa_1 = "<div class='legend-title'> Legenda Mapa UTD {} ({}) ({} suscetíveis a {}) </div>".format(str.lower(st.session_state.utd_suscetiveis_consulta),st.session_state.turma_suscetiveis_consulta,total_total,st.session_state.carteira_suscetiveis_consulta)
                tit_0 = "<li><span style='background:#5C881A;'></span> R$" + menor_0 + "- R$" + maior_0 + "<br>" + total_0 + " " + nome_legenda+ "</li>"
                tit_1 = "<li><span style='background:#0070C0;'></span> R$" + menor_1 + "- R$" + maior_1 + "<br>" + total_1 + " " + nome_legenda+ "</li>"
                tit_2 = "<li><span style='background:#FFC000;'></span> R$" + menor_2 + "- R$" + maior_2 + "<br>" + total_2 + " " + nome_legenda+ "</li>"
                tit_3 = "<li><span style='background:#FF5A00;'></span> R$" + menor_3 + "- R$" + maior_3 + "<br>" + total_3 + " " + nome_legenda+ "</li>"
        ...
        """
            ...
            st.error("Clique em atualizar para plotar o mapa dos contratos suscetíveis")
...
if str.lower(st.session_state.pagina) == "sobre" and st.session_state.check_login == 1:
    col_sobre_0, col_sobre_1 = st.columns([5,95])
    col_sobre_0.write("")
    with col_sobre_1:
        st.markdown("<h4 style='text-align: center; color: #FF5A00;'><em> Atualizações Pendentes:</em></h4>", unsafe_allow_html=True)
        lst = [
        'Inserir visualização do ticket mínimo por UTD, quantidade de suscetíveis, notas esperadas para dia seguinte, zonas que estão sem suscetível.',
        'Inserir visualização da PECLD nas ruas bloqueadas.',  
        'Na aba "'"Seleção"'" -> "'"Defasagem por Zonas"'" inserir a Zona com mesma base fisíca com maior PECLD para eventual seleção em outra base.', 
        'Na aba "'"Seleção"'" -> "'"Seleção D+1 (Zonas)"'" inserir a Zona com mesma base fisíca com maior PECLD para eventual seleção em outra base.', 
        'Bloqueio das abas antes de o login ter sido efetuado.', 
        'Aba com o progesso da seleção do dia (barras de progresso das etapas de seleção).',
        'Inserir legenda no Mapa de Seleção.',
        'Inserir opção de plotar as notas selecionadas com Flag N e Flag S.',
        ]
...
elif st.session_state.pagina == "sobre" and st.session_state.check_login == 0:
    st.error("NECESSÁRIO EFETUAR LOGIN")